...
module.exports = {
  ...
  construct: function(self, options) {
        ...
        function attempt(callback) {
          ...
          function insert(callback) {
            return self.db.insert({
              ...
              unique: self.genex.utils.generateId()
            }, callback);
          }
        }
    ...
    self.withLock = function(name, fn, callback) {
      ...
      function body(callback) {
        ...
        return async.series([ lock, fnWrapper ], function(err) {
          if (locked) {
            return self.genex.locks.unlock(name, function(_err) {
        ...
        function lock(callback) {
          return self.genex.locks.lock(name, function(err) {
            if (err) {
              return callback(err);
            }
            locked = true;
    ...
    self.ensureCollection = function(callback) {
      self.db = self.genex.db.collection('aposLocks');
