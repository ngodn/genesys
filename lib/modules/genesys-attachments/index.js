...
// aspect ratio.
//
// These extra sizes are then available as the `size` option to `genesys-images` widgets
// and when calling `genex.attachments.url`.
//
// ### `imageSizes`
//
...
// means they may appear in any widget that uses images, such as the `genesys-images`
...
// To help you account for this, the CSS class `genex-slideshow-item--svg` is added
...
//
// If you have overridden `widget.html` for `genesys-images-widgets`, view recent commits
...
module.exports = {
  ...
  singletonWarningIfNot: 'genesys-attachments',
  ...
  afterConstruct: function(self, callback) {
    return async.series({
      ...
      self.addPermissions();
      self.genex.tasks.add('genesys-attachments', 'rescale',
        'Usage: node app genesys-attachments:rescale\n\n' +
      ...
      self.genex.tasks.add('genesys-attachments', 'migrate-to-disabled-file-key',
        'Usage: node app genesys-attachments:migrate-to-disabled-file-key\n\n' +
      ...
      self.genex.tasks.add('genesys-attachments', 'migrate-from-disabled-file-key',
        'Usage: node app genesys-attachments:migrate-from-disabled-file-key\n\n' +
      ...
      self.genex.tasks.add('genesys-attachments', 'urls',
  ...
  construct: function(self, options) {
    ...
    var uploadfsDefaultSettings = {
      backend: 'local',
      uploadsPath: self.genex.rootDir + '/public/uploads',
      uploadsUrl: (self.genex.baseUrl || '') + self.genex.prefix + '/uploads',
      tempPath: self.genex.rootDir + '/data/temp/uploadfs',
    ...
    self.apostropheDestroy = function(callback) {
      if (!self.uploadfs.destroy) {
        self.genex.utils.warn('uploadfs dependency is old, cannot clean up resources i.e. timers that will prevent exit');
        return callback(null);
      }
    ...
    self.enableCollection = function(callback) {
      return self.genex.db.collection('aposAttachments', function(err, collection) {
        self.db = collection;
        return callback(err);
      });
    };
