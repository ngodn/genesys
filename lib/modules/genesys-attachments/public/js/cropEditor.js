genex.define('apostrophe-attachments-crop-editor', {
  ...
  construct: function(self, options) {
    ...
    self.afterShow = function() {
      ...
      self.$saveButton = self.$el.find('[data-genex-save]');
      self.busy(true);
      self.$cropImage.attr('src', genex.attachments.url(self.attachment, {size: 'original'}));
    ...
    self.saveContent = function(callback) {
      ...
      self.api('crop', { _id: self.attachment._id, crop: self.attachment.crop }, function(data) {
        ...
        self.busy(false);
        genex.notify('Server error, please retry', { type: 'error' });
        return callback('fail');
      });
    };
