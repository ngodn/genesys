genex.define('apostrophe-attachments', {
  ...
  construct: function(self, options) {
    ...
    self.crop = function(attachment, options, callback) {
      ...
      genex.create('apostrophe-attachments-crop-editor', args);
    };
    ...
    self.focalPoint = function(attachment, options, callback) {
      ...
      _.assign(args, options);
      genex.create('apostrophe-attachments-focal-point-editor', args);
    };

    self.addFieldType = function() {
      genex.schemas.addFieldType({
        name: self.name,
    ...
    self.populate = function(object, name, $field, $el, field, callback) {
      ...
      $input.fileupload({
        ...
        start: function (e) {
          busy(true);
          genex.emit('attachmentUploadStarted', {
            $fieldset: $fieldset,
            field: field
          });
        },
        ...
        always: function (e, data) {
          ...
          if (data.result.status !== 'ok') {
            genex.notify(data.result.status, { type: 'error' });
            genex.emit('attachmentUploadError', {
              $fieldset: $fieldset,
              field: field,
          ...
          if (groups) {
            if (!_.contains(groups, file.group)) {
              ...
              genex.notify("The file must be of one of the following types: %s", extensions.join(', '), { type: 'error' });
              return;
            }
          }
          ...
          genex.emit('attachmentUploadSuccess', {
            $fieldset: $fieldset,
            field: field
          });
    ...
    self.addHandlers = function() {
      genex.ui.link('genex-crop', 'attachment', function($el, _id) {
      ...
      genex.ui.link('genex-focal-point', 'attachment', function($el, _id) {
        var $existing = $el.closest('[data-existing]');
    ...
    self.updateExisting = function($fieldset, info, field) {
      ...
      $existing.alterClass('genex-extension-*', 'genex-extension-' + info.extension);
      var $preview = $existing.find('[data-preview]');
      if (info.group === 'images') {
        $preview.attr('src', self.url(info, { size: 'one-half', crop: info.crop }));
        $fieldset.find('[data-genex-crop-attachment]').show();
        $fieldset.find('[data-genex-focal-point-attachment]').show();
      } else {
        $preview.hide();
        $fieldset.find('[data-genex-crop-attachment]').hide();
        $fieldset.find('[data-genex-focal-point-attachment]').hide();
      }
      $existing.show();
      genex.emit('attachmentUpdated', {
        $fieldset: $fieldset,
        attachment: info,
        field: field
      });
