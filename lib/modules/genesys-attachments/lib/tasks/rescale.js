        ...
        function(file, fileCallback) {
          ...
          async.series([
            function(callback) {
              ...
              tempFile = self.uploadfs.getTempPath() + '/' + self.genex.utils.generateId() + '.' + file.extension;
              n++;
              ...
              async.series([
                ...
                function(callback) {
                  return self.uploadfs.copyOut(originalFile, tempFile, function(err) {
                    if (err) {
                      self.genex.utils.error('WARNING: could not access ' + originalFile + ', perhaps it was deleted');
                      return fileCallback(null);
                    }
                ...
                function(callback) {
                  if (!argv['crop-only']) {
                    return self.uploadfs.copyImageIn(tempFile, originalFile, function(err) {
                      if (err) {
                        self.genex.utils.error('WARNING: could not work with ' + tempFile + ' even though copyOut claims it is there');
                        return fileCallback(null);
                      }
                      return callback(null);
                    });
