...
module.exports = function(self, options) {
  ...
  self.insert = function(req, file, options, callback) {
    ...
    function body(callback) {
      ...
      var info = {
        _id: self.genex.utils.generateId(),
        group: group.name,
        createdAt: new Date(),
        name: self.genex.utils.slugify(path.basename(file.name, path.extname(file.name))),
        title: self.genex.utils.sortify(path.basename(file.name, path.extname(file.name))),
        extension: extension,
        type: 'attachment',
        docIds: [],
        trashDocIds: []
      ...
      function permissions(callback) {
        ...
        return callback(self.genex.permissions.can(req, 'edit-attachment') ? null : 'forbidden');
      }

      function length(callback) {
        return self.genex.utils.fileLength(file.path, function(err, size) {
          if (err) {
            return callback(err);
          }
          info.length = size;
      ...
      function md5(callback) {
        return self.genex.utils.md5File(file.path, function(err, md5) {
          if (err) {
            return callback(err);
          }
          info.md5 = md5;
          ...
          return self.uploadfs.copyImageIn(file.path, targetPath, function(err, result) {
            ...
            if (info.extension === 'jpg') {
              return exif.parse(file.path, (err, exifData) => {
                if (err) {
                  self.genex.utils.error('jpeg-exif error:', err);
                  return callback(null);
                }
      ...
      function remember(callback) {
        if ((!options) || (options.permissions !== false)) {
          info.ownerId = self.genex.permissions.getEffectiveUserId(req);
        }
  ...
  self.crop = function(req, _id, crop, callback) {
    ...
    return async.series([
      ...
      var tempFile = self.uploadfs.getTempPath() + '/' + self.genex.utils.generateId() + '.' + info.extension;
  ...
  self.sanitizeCrop = function(crop) {
    ...
    crop.top = self.genex.launder.integer(crop.top, 0, 0, 10000);
    crop.left = self.genex.launder.integer(crop.left, 0, 0, 10000);
    crop.width = self.genex.launder.integer(crop.width, 1, 1, 10000);
    crop.height = self.genex.launder.integer(crop.height, 1, 1, 10000);
  ...
  self.clone = function(req, source, callback) {
    ...
    var tempFile = self.uploadfs.getTempPath() + '/' + self.genex.utils.generateId() + '.' + source.extension;
  ...
  self.getMissingAttachmentUrl = function() {
    var defaultIconUrl = '/modules/genesys-attachments/img/missing-icon.svg';
    self.genex.utils.warn('Template warning: Impossible to retrieve the attachment url since it is missing, a default icon has been set. Please fix this ASAP!');
    return defaultIconUrl;
  };

  // This method is available as a template helper: genex.attachments.url
  //
  ...
  // This method is available as a template helper: genex.attachments.first
  //
  ...
  //
  // genex.attachments.first(page.body)
  //
  // 2. Must be a PDF
  //
  // genex.attachments.first(page.body, { extension: 'pdf' })
  //
  // 3. May be any office-oriented file type
  //
  // genex.attachments.first(page.body, { group: 'office' })
  //
  // genex.images.first is a convenience wrapper for fetching only images.
  //
  // OPTIONS:
  //
  ...
  // This method is available as a template helper: genex.attachments.all
  //
  ...
  // 1. In the body please
  //
  // genex.attachments.all(page.body)
  //
  // 2. Must be a PDF
  //
  // genex.attachments.all(page.body, { extension: 'pdf' })
  //
  // 3. May be any office-oriented file type
  //
  // genex.attachments.all(page.body, { group: 'office' })
  //
  // genex.images.all is a convenience wrapper for fetching only images.
  //
  // OPTIONS:
  //
  // You may specify `extension`, `extensions` (an array of extensions)
  ...
  self.all = function(within, options) {
    ...
    self.genex.docs.walk(within, function(o, key, value, dotPath, ancestors) {
      if (test(value)) {
        ...
        // that cropping coordinates stored in an genesys-images widget
        // are passed through when we make a simple call to
        // genex.attachments.url with the returned object
        var i;
  ...
  self.each = function(criteria, limit, each, callback) {
    ...
    // tasks like genesys-attachments:rescale. We need a robust solution that
  ...
  self.middleware = {
    canUpload: function(req, res, next) {
      if (!self.genex.permissions.can(req, 'edit-attachment')) {
        res.statusCode = 403;
  ...
  self.addTypeMigration = function() {
    ...
    self.genex.migrations.add(self.__meta.name + '.addType', function(callback) {
      ...
      function docs(callback) {
        ...
        return self.genex.migrations.eachDoc({}, function(doc, callback) {
          var changed = false;
          self.genex.docs.walk(doc, function(o, key, value) {
          ...
          self.genex.docs.db.update({ _id: doc._id }, doc, callback);
        }, callback);
      }
    }, {
      safe: true
  ...
  self.addDocReferencesMigration = function() {
    ...
    self.genex.migrations.add(self.__meta.name + '.docReferencesContained', function(callback) {
  ...
  // `genesys-attachments:recompute-doc-references`
  ...
  self.recomputeAllDocReferences = function(callback) {
    ...
    function docs(callback) {
      return self.genex.migrations.eachDoc({}, 5, addAttachmentUpdates, callback);
    }
  ...
  self.addFixPermissionsMigration = function() {
    self.genex.migrations.add(self.__meta.name + '.fixPermissions', function(callback) {
      return self.genex.migrations.each(self.db, { group: 'images', trash: true }, 5, function(attachment, callback) {
        return self.applyPermissions(attachment, true, callback);
      }, function(err) {
        if (err) {
          return callback(err);
        }
        self.genex.utils.warn('^^^ WARNINGS ABOVE ARE OK. This migration fixes a scenario in which SOME permissions were not set.');
        return callback(null);
      });
    }, {
      safe: true
  ...
  self.addResetUploadfsPermissionsTask = function() {
    self.addTask('reset-uploadfs-permissions', 'Reset the permissions of files in uploadfs to match the trash flag of each attachment', function(genex, argv, callback) {
      return self.resetUploadfsPermissions(callback);
    });
  };

  self.resetUploadfsPermissions = function(callback) {
    return self.genex.migrations.each(self.db, {}, 5, function(attachment, callback) {
      return self.applyPermissions(attachment, attachment.trash, callback);
    }, function(err) {
      if (err) {
        return callback(err);
      }
      self.genex.utils.warn('^^^ WARNINGS ABOVE ARE OK. You may see some in cases where the permissions are\nalready correct and cannot be set the same way twice in a row.');
      return callback(null);
    });
  };
  ...
  self.updateDocReferences = function(doc, callback) {
    ...
    doc = self.genex.utils.clonePermanent(doc);

    var attachments = self.all(self.genex.utils.clonePermanent(doc));
      ...
      return async.eachSeries(sizes, function(size, callback) {
        ...
        return method(path, function(err) {
          if (err) {
            ...
            self.genex.utils.warn('Unable to set permissions on ' + path + ', most likely already done');
          }
          return callback(null);
        });
      }, callback);
    ...
    function cropOne(crop, callback) {
      return async.eachSeries(self.imageSizes.concat([ { name: 'original' } ]), function(size, callback) {
        ...
        return method(path, function(err) {
          if (err) {
            // afterSave is not a good place for fatal errors
            self.genex.utils.warn('Unable to set permissions on ' + path + ', possibly it does not exist');
          }
          return callback(null);
        });
      }, callback);
  ...
  self.urlsTask = function(callback) {
    var uploadfsPath = !!self.genex.argv['uploadfs-path'] || !!self.genex.argv['uploads-url'];
    var seen = {};
    return self.genex.migrations.each(self.db, {}, 1, function(attachment, callback) {
      url(attachment);
      _.each(self.imageSizes, function(size) {
    ...
    function url(attachment, options) {
      ...
      if (self.genex.argv['uploads-url']) {
        url = self.genex.argv['uploads-url'] + url;
      }
      if (seen[url]) {
        return;
      }
