// The `genesys-users` module provides user accounts. It is **not** intended to
// be extended with new subclass modules. The `genesys-login` module only
// looks for instances of `genesys-user`. Of course you may implicitly subclass
...
// join with `genesys-group` in the schema.
//
// Groups are managed by the `genesys-groups` module.
//
// There is also a simplified permissions model in which you just specify
// an array of `groups` as an option to `genesys-users`, and a single-select
...
// `genesys-users`, the admin interface for `genesys-groups` will hide itself.
//
...
// `genesys-users`. You can add it back in, but then you have to deal with
...
// or similar, extending `genesys-pieces`, unless it's true that basically
...
//
// You may also call `genex.users.addSecret('name')` to add a new
// secret property. This is convenient when implementing a module
// such as `genesys-signup`.
...
module.exports = {
    ...
    options.arrangeFields = [
      {
        name: 'basics',
        label: 'Basics',
        fields: [ 'firstName', 'lastName', 'title', 'username', 'email', 'password', 'slug', 'group', '_groups', 'disabled', 'slug' ].concat(options.genex.docs.trashInSchema ? [ 'trash' ] : [])
      }
  ...
  construct: function(self, options) {
    ...
    self.ensureSafeCollection = function(callback) {
      return self.genex.db.collection('aposUsersSafe', function(err, collection) {
    ...
    // an `genesys-user` and, if so, hash the password, remove it from the doc
    ...
    // First checks to be sure this is an `genesys-user` and invokes
    ...
    // hash in the safe. `user` is an `genesys-user` doc.
    ...
    self.ensureGroup = function(group, callback) {
      ...
      return self.genex.groups.find(req, criteria).toObject(function(err, _group) {
        if (err) {
          return callback(err);
        }
        if (_group) {
          ...
          return self.genex.groups.update(req, _group, function(err) {
            if (err) {
              return callback(err);
            }
        ...
        return self.genex.groups.insert(req, group, function(err) {
          if (err) {
            return callback(err);
          }
    ...
    self.genex.tasks.add('genesys-users', 'add',
      'Usage: node app genesys-users:add username groupname\n\n' +
    ...
    // Implement the `genesys-users:add` command line task.

    self.addFromTask = function(callback) {
      var argv = self.genex.argv;
      if (argv._.length !== 3) {
      ...
      var req = self.genex.tasks.getReq();
      // find the group
      self.genex.groups.find(req, {title: groupname}).toObject(function(err, group) {
        if (err) {
          return callback(err);
        }
        if (!group) {
    ...
    self.genex.tasks.add('genesys-users', 'change-password',
      'Usage: node app genesys-users:change-password username\n\n' +
    ...
    // Implement the `genesys-users:change-password` task.
    ...
    self.changePasswordFromTask = function(callback) {
      ...
      function findUser(callback) {
        return self.genex.users.find(req, { username: username }).toObject(function(err, _user) {
          if (err) {
            return callback(err);
          }
          if (!_user) {
    ...
    self.apiRoute('post', 'unique-username', function(req, res, next) {
      var username = self.genex.launder.string(req.body.username);
    ...
    self.addNonNullJoinMigration = function() {
      self.genex.migrations.add(self.__meta.name + ':non-null-joins', function() {
        var fields = {
          'viewUsersIds': [],
          'viewGroupsIds': [],
          'editUsersIds': [],
        ...
        return Promise.mapSeries(_.keys(fields), function(name) {
          ...
          return self.genex.docs.db.update(criteria, {
            $set: $set
          }, {
            multi: true
          });
