// `genesys-pieces-widgets` provides widgets that display display pieces of a
...
module.exports = {
  ...
  beforeConstruct: function(self, options) {
    ...
    self.pieces = options.genex.modules[self.piecesModuleName];
    if (!self.pieces) {
      if (!options.piecesModuleName) {
        throw new Error('The module ' + self.__meta.name + ' extends genesys-pieces-widgets, but the piecesModuleName option is not set, and we can\'t guess it from the name of your module.');
      } else {
  ...
  construct: function(self, options) {
    ...
    self.load = function(req, widgets, callback) {
      // Since we are overriding the default load method of `genesys-widgets`, make
    ...
    self.loadOtherJoins = function(req, widgets, callback) {
      ...
      return self.genex.schemas.join(req, schema, widgets, undefined, callback);
    };
    ...
    self.orderPiecesForWidget = function(widget) {
      ...
      widget._pieces = self.genex.utils.orderById(widget.pieceIds, widget._pieces, key);
    };
