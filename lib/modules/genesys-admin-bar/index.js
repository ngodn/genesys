...
// particular permissions. [genesys-pieces](../genesys-pieces/README.md) automatically
...
module.exports = {
  ...
  // that provides `genex.adminBar.link`. Most server-side initialization happens
  ...
  construct: function(self, options) {
    ...
    //
    // The name argument becomes the value of the `data-genex-admin-bar-item`
    // attribute of the admin bar item.
    //
    // `permission` should be a permission name such as `admin`
    // (the user must be a full admin) or `edit-genesys-event`
    ...
    //
    // ```
    // self.genex.adminBar.add(self.__meta.name, self.pluralLabel, 'edit')
    // ```
    //
    // If you have an `events` module that subclasses pieces, then this
    // creates an admin bar item with a data-genex-admin-item="events" attribute.
    //
    // Browser side, you can call `genex.adminBar.link('item-name', function() { ...})`
    ...
    self.addHelpers({
      ...
      output: function() {
        ...
        var user = self.genex.templates.contextReq.user;
        if (!user) {
          return '';
        }
        var items = _.filter(self.items, function(item) {
          return self.itemIsVisible(self.genex.templates.contextReq, item);
        });
        if (!items.length) {
          return '';
        }
    ...
    self.pushAssets = function() {
      ...
      var closeDelay = (self.options.closeDelay || self.options.closeDelay === 0 ? self.genex.launder.integer(self.options.closeDelay) : null);
      ...
      self.genex.push.browserCall('user', 'genex.create(?, ?)', self.__meta.name, options.browser);
    };
    ...
    self.itemIsVisible = function(req, item) {
      ...
      return self.genex.permissions.can(req, item.permission);
    };
  }
};
