...
module.exports = function(self, options) {
  self.renderRoute('post', 'versions-editor', function(req, res, next) {
    ...
    return async.series({
      findDoc: function(callback) {
        return self.genex.docs.find(req, { _id: _id }).published(null).permission('edit').toObject(function(err, _doc) {
          if (err) {
            return callback(err);
          }
          if (!_doc) {
  ...
  self.apiRoute('post', 'compare', function(req, res, next) {
    var oldId = self.genex.launder.id(req.body.oldId);
    var currentId = self.genex.launder.id(req.body.currentId);
    var current;
    return async.series({
      findVersions: function(callback) {
  ...
  self.apiRoute('post', 'revert', function(req, res, next) {
    ...
    return async.series({
      fetch: function(callback) {
        return self.find(req, { _id: self.genex.launder.id(req.body._id) }, {}, function(err, versions) {
          if (err) {
            return callback(err);
          }
          if (!versions[0]) {
