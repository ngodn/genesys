...
module.exports = function(self, options) {
  self.enableCollection = function(callback) {
    return self.genex.db.collection('aposDocVersions', function(err, collection) {
      self.db = collection;
      return callback(err);
    });
  };
  ...
  self.docAfterSave = function(req, doc, options, callback) {
    ...
    var pruned = self.genex.utils.clonePermanent(doc);
    var version = {
  ...
  self.pruneOldVersions = function(doc, callback) {
    ...
    function iterator(err, version) {
      if (err) {
        self.genex.utils.error('An error occurred while pruning versions.');
      ...
      return self.db.remove({ _id: version._id }, function(err) {
        if (err) {
          self.genex.utils.error('An error occurred while pruning versions (remove)');
          self.genex.utils.error(err);
        }
        return cursor.nextObject(iterator);
      });
  ...
  self.revert = function(req, version, callback) {
    ...
    return async.series({
      ...
      return self.genex.docs.update(req, newDoc, callback);
    });
  };
  ...
  self.find = function(req, criteria, options, callback) {
    ...
    return async.series({
      ...
      permissions: function(callback) {
        ...
        var docId = versions[0].docId;
        return self.genex.docs.find(req, { _id: docId }).published(null).permission('edit').toObject(function(err, doc) {
          if (err) {
            return callback(err);
          }
          if (!doc) {
  ...
  self.compare = function(req, doc, version1, version2, callback) {
    ...
    return async.eachSeries(docTypes, function(docType, callback) {
      ...
      var manager = self.genex.docs.getManager(docType);
      if (!manager) {
        return callback(null);
      }
    ...
    function compareObjects(schema, version1, version2) {
      ...
      function compareField(field, old, current) {
        ...
        if (field.type === 'array') {
          ...
          var currentLines = [];
          var currentText = '';
          var fieldType = self.genex.schemas.getFieldType(field.type);
    ...
    function compareAreas(version1, version2) {
      ...
      _.each(version1.items, function(widget1) {
        var manager = self.genex.areas.getWidgetManager(widget1.type);
        if (!manager) {
      ...
      _.each(version2.items, function(widget2) {
        var manager = self.genex.areas.getWidgetManager(widget2.type);
        if (!manager) {
          // No warning message here because it may have been removed deliberately
          // from a later version of the site
          return;
      ...
      if ((version1.items && version2.items && version1.items.length === version2.items.length) && (!importantChanges)) {
        ...
        var moved = _.find(version2.items, function(widget2, i) {
          if (self.genex.areas.getWidgetManager(widget2.type) && (ranksById[widget2._id] !== i)) {
        ...
        if (moved) {
          changes.push({
            ...
            current: moved,
            manager: self.genex.areas.getWidgetManager(moved.type)
          });
        }
      }
      return changes;
