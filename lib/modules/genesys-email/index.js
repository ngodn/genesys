...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    //
    // See `genesys-module` for coverage of the `email` method that
    ...
    // configured for the `genesys-email` module. If there is none,
    // a fatal error is thrown.

    self.getTransport = function() {
      if (!self.transport) {
        if (!self.options.nodemailer) {
          throw new Error('The nodemailer option must be configured for the genesys-email module before any module can call its email() method. The option accepts an object to be passed to nodemailer.createTransport, per the nodemailer documentation.');
        }
