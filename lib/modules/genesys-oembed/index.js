...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    // Create the browser-side object `genex.oembed` for convenient oembed queries
    // and display of oembed responses. Called by `afterConstruct`.

    self.pushCreateSingleton = function() {
      self.genex.push.browserCall('always', 'genex.create("apostrophe-oembed", ?)', { action: self.action });
    };
    ...
    self.createRoutes = function() {
      ...
      self.route('get', 'query', function(req, res) {
        var url = self.genex.launder.string(req.query.url);
        var options = {
          alwaysIframe: self.genex.launder.boolean(req.query.alwaysIframe),
          iframeHeight: self.genex.launder.integer(req.query.iframeHeight),
          neverOpenGraph: self.genex.launder.boolean(req.query.neverOpenGraph)
        };
