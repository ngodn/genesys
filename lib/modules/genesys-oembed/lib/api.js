...
module.exports = function(self, options) {
  ...
  self.query = function(req, url, options, mainCallback) {
    ...
    if (!self.cache) {
      self.cache = self.genex.caches.get('oembed');
    }
    var response;
    // Tolerant URL handling
    url = self.genex.launder.url(url);
    if (!url) {
  ...
  self.openGraph = function(req, url, callback) {
    return request(url, function(err, response, body) {
      ...
      description = self.genex.utils.truncatePlaintext(description, 300);
  ...
  self.iframe = function(req, url, options, callback) {
    return request(url, function(err, response, body) {
      ...
      var style = '';
      var html = '<iframe STYLE src="' + self.escapeHtml(url) + '" class="genex-always-iframe"></iframe>';
