...
module.exports = function(self, options) {
  ...
  self.listTags = function(req, options, callback) {
    ...
    if (options.prefix && options.prefix.length) {
      criteria.tags = new RegExp('^' + self.genex.utils.regExpQuote(options.prefix));
    } else if (options.contains && options.contains.length) {
      criteria.tags = new RegExp(self.genex.utils.regExpQuote(options.contains));
    } else if (options.all) {
    ...
    self.genex.docs.find(req, criteria).toDistinct('tags', function(err, tags) {
      if (err) {
        return callback(err);
      }
  ...
  self.addTag = function(req, tag, callback) {
    var piece = {
      title: tag,
      slug: self.genex.utils.slugify('tag-' + tag),
      tags: [ tag ],
      published: true
    };
  ...
  self.renameTag = function(req, tag, newTag, callback) {
    ...
    return async.series({
      addToSet: function(callback) {
        return self.genex.docs.db.update(criteria, { $addToSet: { tags: newTag } }, { multi: true }, callback);
      },
      pull: function(callback) {
        return self.genex.docs.db.update(criteria, { $pull: { tags: tag } }, { multi: true }, callback);
      },
      findPiece: function(callback) {
        return self.find(req, { slug: self.genex.utils.slugify('tag-' + tag) })
      ...
      updatePiece: function(callback) {
        ...
        piece.title = newTag;
        piece.slug = self.genex.utils.slugify('tag-' + newTag);
  ...
  self.deleteTag = function(req, tag, callback) {
    ...
    return async.series({
      ...
      deletePiece: function(callback) {
        ...
        return self.genex.docs.db.remove({ _id: piece._id }, callback);
      }
    }, function(err) {
      return callback(err);
    });
  ...
  self.launder = function(tag) {
    return self.genex.launder.filterTag(self.genex.launder.string(tag));
  };
