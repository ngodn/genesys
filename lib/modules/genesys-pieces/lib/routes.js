...
module.exports = function(self, options) {
  ...
  self.createRoutes = function() {
    ...
    self.route('post', 'permissions', self.routes.permissions);
    ...
    self.route('post', 'insert-via-upload', self.genex.attachments.middleware.canUpload, self.genex.middleware.files, self.routes.insertViaUpload);
  };
  ...
  self.routes.list = function(req, res) {
    ...
    return async.series({
      ...
      if (options.format === 'managePage') {
        ...
        var viewTemplate = 'manage' + self.genex.utils.capitalizeFirst(view) + 'View';
        results = {
  ...
  self.routes.permissions = function(req, res) {
    ...
    const permissions = {};
    return self.genex.schemas.convert(req, schema, 'form', req.body, permissions, function(err) {
      if (err) {
        self.genex.utils.error(err);
  ...
  self.routes.trash = function(req, res) {
    ...
    if (req.body.job) {
      // New way
      return self.genex.modules['apostrophe-jobs'].run(req, function(req, id, callback) {
    ...
    if (Array.isArray(req.body.ids)) {
      ids = self.genex.launder.ids(req.body.ids);
    } else {
      ids = [ self.genex.launder.id(req.body._id) ];
    }
  ...
  self.routes.insertViaUpload = function(req, res) {
    ...
    if (!field) {
      self.genex.utils.error('The pieces module ' + self.options.__meta.name + ' has insertViaUpload turned on but the schema does not contain a field of type attachment.');
      res.statusCode = 404;
      return res.send('notfound');
    }
    ...
    function acceptFile(callback) {
      return self.genex.attachments.insert(req, file, function(err, _attachment) {
        if (err) {
          return callback(err);
        }
        attachment = _attachment;
        if (!self.genex.attachments.acceptableExtension(field, attachment)) {
