...
genex.define('genesys-pieces-manager-modal', {
  ...
  construct: function(self, options) {
    ...
    self.beforeShow = function(callback) {
      self.$manageView = self.$el.find('[data-genex-manage-view]');
    ...
    self.enableFilters = function() {
      ...
      _.each(self.filters, function(filter) {
        ...
        self.link('genex-' + filter.name, function($el, value) {
    ...
    self.enableInsertViaUpload = function() {
      ...
      getUploader().fileupload({
        ...
        fail: function (e, data) {
          ...
          if (data.result.status === 'ok') {
            ...
            genex.change(self.options.name);
      ...
      self.$controls.on('click', '[data-genex-upload-' + self.options.name + ']', function() {
      ...
      function feedback(data) {
        if (getUploader().fileupload('active') <= 1) {
          if (self.insertViaUploadStatus.errors) {
            genex.notify('Some files were damaged, of an unsuitable type or too large to be uploaded.', { type: 'error' });
          } else if (self.insertViaUploadStatus.unsuitable) {
            genex.notify('Some uploaded files were unsuitable for the current placement.', { type: 'error' });
          } else {
      ...
      function busy(state) {
        genex.ui.globalBusy(state);
      }

      function getUploader() {
        return self.$controls.find('[data-genex-uploader-' + self.options.name + ']');
      }
    };
    ...
    self.reflectSort = function() {
      ...
      $columns.removeClass('genex-manage-column--forward')
        .removeClass('genex-manage-column--backward')
        .removeClass('genex-manage-column--next-forward')
        .removeClass('genex-manage-column--next-backward')
        .removeClass('genex-manage-column--next-none');
      ...
      if (self.sort) {
        ...
        if (self.sort.direction === '1') {
          $current.addClass('genex-manage-column--forward');
        } else {
          $current.addClass('genex-manage-column--backward');
        }
      ...
      function indicateNext($column, direction) {
        if (direction === '1') {
          $column.addClass('genex-manage-column--next-forward');
        } else if (direction === '-1') {
          $column.addClass('genex-manage-column--next-backward');
        } else {
          $column.addClass('genex-manage-column--next-none');
        }
      }
    };

    self.enableChooseViews = function() {
      self.link('genex-choose-manage-view', function($el, value) {
        self.viewName = value;
        self.refresh();
      });
    };
    ...
    self.enableCheckboxEvents = function() {
      ...
      self.$el.on('click', '.genex-field-input-checkbox-indicator', function (e) {
        var box = $(this).siblings('.genex-field-input-checkbox')[0];
    ...
    self.shrinkGrid = function() {
      self.$el.find('.genex-modal-footer').css('bottom', '72px');
      self.$el.find('.genex-modal-body, .genex-chooser').css('height', 'calc(100% - 144px)');
    };

    self.growGrid = function() {
      self.$el.find('.genex-modal-footer').css('bottom', '0px');
      self.$el.find('.genex-modal-body, .genex-chooser').css('height', 'calc(100% - 72px)');
    };
    ...
    self.reflectSelectEverything = function () {
      ...
      if ($pieces.length === $checked.length) {
        ...
        self.shrinkGrid();
        if (!$selectEverything.hasClass('genex-active')) {
        ...
        self.growGrid();
        $selectEverything.removeClass('genex-active');
    ...
    self.clearChoices = function() {
      self.choices = [];
      self.$el.removeClass('genex-manager-modal--has-choices');
      self.$el.addClass('genex-manager-modal--has-no-choices');
      self.reflectChoiceCount();
    };
    ...
    self.refreshSelectEverything = function() {
      ...
      return self.api('list', listOptions, function(response) {
        ...
        self.getSelectEverything().addClass('genex-active');
        self.reflectSelectEverythingCheckbox();
      });
    };
    ...
    self.reflectHasChoices = function() {
      if (!self.getIds().length) {
        ...
        self.$el.addClass('genex-manager-modal--has-choices');
        self.$el.removeClass('genex-manager-modal--has-no-choices');
      }
    };
    ...
    self.refresh = function(callback) {
      ...
      return self.api('list', listOptions, function(response) {
        ...
        genex.emit('enhance', self.$filters);
        genex.emit('enhance', self.$manageView);
        genex.emit('enhance', self.$pager);
    ...
    self.afterHide = function() {
      ...
      // after we're gone
      genex.off('change', self.onChange);
    };
