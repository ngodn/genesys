...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.addSearchTexts = function(item, texts) {
      texts.push({ weight: 10, text: self.genex.utils.htmlToPlaintext(item.content), silent: false });
    };
    ...
    self.compare = function(old, current) {
      ...
      var diff = jsDiff.diffSentences(self.genex.utils.htmlToPlaintext(oldContent).replace(/\s+/g, ' '), self.genex.utils.htmlToPlaintext(currentContent).replace(/\s+/g, ' '), { ignoreWhitespace: true });
    ...
    self.isEmpty = function(widget) {
      var text = self.genex.utils.htmlToPlaintext(widget.content || '');
