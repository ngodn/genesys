genex.define('apostrophe-rich-text-widgets', {
  construct: function(self, options) {
    ...
    self.edit = function(data, options, save) {
      ...
      return save(data, function(err, $widget) {
        if (err) {
          ...
          genex.notify('An error occurred saving the text.', { type: 'error' });
          return;
        }
    ...
    self.startEditing = function($widget) {
      ...
      self.startAutosavingAreaThen($widget, function() {
        $widget.removeClass('genex-empty');
        if ($widget.data('editor')) {
          $widget.data('editor').start();
          return;
        }
        var options = genex.areas.getWidgetOptions($widget);
        options.$widget = $widget;
        var instance = genex.create('apostrophe-rich-text-widgets-editor', options);
