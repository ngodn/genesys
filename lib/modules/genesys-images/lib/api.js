...
module.exports = function(self, options) {
  ...
  // This method is available as a template helper: genex.images.first
  //
  ...
  // an `genesys-images` piece or not. It simply checks whether the `group`
  ...
  //
  // genex.images.first(page.body)
  //
  // 2. Must be a GIF
  //
  // genex.images.first(page.body, { extension: 'gif' })
  //
  ...
  self.first = function(within, options) {
    ...
    var result = self.genex.attachments.first(within, options);
    return result;
  };

  // This method is available as a template helper: genex.images.all
  //
  ...
  //
  // genex.images.all(page.body)
  //
  // 2. Must be GIFs
  //
  // genex.images.all(page.body, { extension: 'gif' })
  //
  // (Note Apostrophe always uses .jpg for JPEGs.)
  //
  // OPTIONS:
  ...
  self.all = function(within, options) {
    options = options ? _.clone(options) : {};
    options.group = 'images';
    return self.genex.attachments.all(within, options);
  };

  // This method is available as a template helper: genex.images.srcset
  //
  ...
  self.srcset = function(attachment, cropRelationship) {
    if (!self.genex.attachments.isSized(attachment)) {
      return '';
    }
    ...
    var sources = self.genex.attachments.imageSizes.filter(function(imageSize) {
      ...
      var src = self.genex.attachments.url(attachment, { size: imageSize.name, crop: cropRelationship });
  ...
  self.isCroppable = function(image) {
    return image && self.genex.attachments.isCroppable(image.attachment);
  };
  ...
  self.afterList = function(req, results, callback) {
    ...
    if (minSize) {
      results.minSize = [
        self.genex.launder.integer(minSize[0]),
        self.genex.launder.integer(minSize[1])
      ];
    }
