...
genex.define('genesys-images-chooser', {
  extend: 'genesys-pieces-chooser',
  autocomplete: false,
  construct: function(self, options) {
    self.finalize = function(callback) {
      ...
      var manager = genex.docs.getManager(self.field.withType);
        ...
        return async.eachSeries(choices, function(choice, callback) {
          ...
          function retrieve(callback) {
            return manager.api('retrieve', { _id: choice.value }, function(result) {
              if (result.status !== 'ok') {
                genex.notify('An error occurred while retrieving the image to be cropped. Perhaps it has been removed.', { type: 'error' });
                return;
              }
          ...
          function crop(callback) {
            if (!genex.attachments.options.croppable[attachment.extension]) {
              return callback(null);
            }
            ...
            return genex.attachments.api(
              ...
              function(data) {
                if (data.status !== 'ok') {
                  genex.notify('An error occurred while cropping.', { type: 'error', dismiss: true });
    ...
    self.link('genex-focal-point', 'item', function($button, _id) {
