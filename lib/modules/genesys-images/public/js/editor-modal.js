genex.define('genesys-images-editor-modal', {
  ...
  construct: function(self, options) {
    ...
    self.enableTitleFromAttachment = function() {
      genex.on('attachmentUpdated', self.titleFromAttachmentListener);
    };
    ...
    self.titleFromAttachmentListener = function(info) {
      if (!self.$el.is(':visible')) {
        ...
        genex.off('attachmentUpdated', self.titleFromAttachmentListener);
        return;
      }
      ...
      var $title = genex.schemas.findField(self.$el, 'title');
      if (!$title.val().length) {
