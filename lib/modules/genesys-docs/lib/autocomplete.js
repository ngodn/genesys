...
module.exports = function(self) {
  ...
  return function(callback) {
    ...
    autocomplete = self.genex.utils.sortify(autocomplete);
    ...
    clauses = clauses.concat(
      _.map(words, function(word) {
        return {
          highSearchWords: self.genex.utils.searchify(word, true)
        };
      })
    );

    clauses.push(
      { highSearchText: self.genex.utils.searchify(autocomplete) }
    );
