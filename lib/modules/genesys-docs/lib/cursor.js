...
// An `genesys-cursor` allows you to conveniently fetch docs from
// the `aposDocs` mongodb collection using chainable filters, much
// like a MongoDB or Doctrine cursor.
//
// Usually you'll be working with a subclass of `genesys-cursor`
...
// main `genesys-cursor` class that are useful with all types
...
// `find()` method of the `genesys-docs` module (`genex.docs.find`).
// If you are interested in all pages (docs that are part of the
// page tree), call the `find()` method of the `genesys-pages`
// module (`genex.pages.find`).
//
...
// `genesys-blog` module is in use)
//
// ```
// return genex.docs.getManager('genesys-blog').
//   find().
//   limit(10).
...
// on an `genesys-pieces-pages` page, such as a blog. In many
...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    // in any way (hint: use `self.genex.launder.string`, etc).
    // If you also set `safeFor: 'public'`, the filter then becomes
    // available as a query string parameter in `genesys-pieces-pages`
    ...
    self.projectUrlField = function(add) {
      ...
      var manager = type && self.genex.docs.getManager(type);
      var fields;
      if (manager) {
        fields = manager.getUrlFields();
      } else {
        fields = self.genex.docs.getDefaultUrlFields();
      }
    ...
    // generic cursor obtained from `genex.docs.find`
    ...
    // a default.
    //
    // This filter is called by genesys-pieces based on its
    ...
    self.addFilter('skip', {
      launder: function(s) {
        return self.genex.launder.integer(s, 0, 0);
      },
      safeFor: 'public'
    });
    ...
    //
    // Used by `genesys-pieces` and `genesys-pieces-pages`.

    self.addFilter('perPage', {
      def: undefined,
      launder: function(i) {
        return self.genex.launder.integer(i, 1, 1);
      },
      safeFor: 'manage'
    });
    ...
    self.addFilter('permission', {
      finalize: function() {
        ...
        if (permission !== false) {
          ...
          var criteria = self.genex.permissions.criteria(self.get('req'), permission);
          self.and(criteria);
        }
      },
      after: function(results) {
        ...
        self.genex.permissions.annotate(self.get('req'), 'edit-doc', results);
        self.genex.permissions.annotate(self.get('req'), 'publish-doc', results);
      }
    });
    ...
    self.addFilter('tags', {
      ...
      launder: function(s) {
        ...
        return _.map(s, function(value) {
          return self.genex.launder.string(value);
        });
      }
    });
    ...
    self.addFilter('trash', {
      ...
      safeFor: 'manage',
      launder: function(s) {
        return self.genex.launder.booleanOrNull(s);
      },
    ...
    self.addFilter('explicitOrder', {
      ...
      after: function(results) {
        ...
        var property = self.get('explicitOrderProperty');
        var temp = self.genex.utils.orderById(values, results, property);
        var i;
    ...
    self.addFilter('joins', {
      ...
      after: function(results, callback) {
        ...
        return async.eachSeries(_.keys(resultsByType), function(type, callback) {
          ...
          return self.genex.schemas.join(self.get('req'), manager.schema, resultsByType[type], self.get('joins'), callback);
        }, callback);
      }
    });
    ...
    self.addFilter('addUrls', {
      ...
      after: function(results, callback) {
        ...
        return async.eachSeries(interesting, function(type, callback) {
          return self.genex.docs.getManager(type).addUrls(req, byType[type], callback);
        }, callback);
      }
    });
    ...
    // genesys-pieces-pages automatically does this when a filter is
        ...
        return async.waterfall([
          ...
          function(callback) {
            ...
            if (self.get('log') || process.env.APOS_LOG_ALL_QUERIES) {
              self.genex.utils.log(require('util').inspect(criteria, { depth: 20 }));
            }
    ...
    // It is a low level implementation detail overridden by `genesys-optimizer` as needed.
    // Seemingly we don't need req at all here, but overrides like genesys-optimizer need it,
    ...
    self.clone = function() {
      var cursor = self.genex.create(self.__meta.name, options);
    ...
    self.finalizeSort = function() {
      ...
      if (sort) {
        ...
        // There's an genesys-page "type" that is never really in the database but
        // has a manager thanks to genesys-any-page-manager. Use that as a default
        // so that we always get a manager object

        var manager = self.genex.docs.getManager(self.get('type') || 'genesys-page');
        if (!(manager && manager.schema)) {
          return;
        }
