...
module.exports = function(self, options) {
  ...
  self.enableCollection = function(callback) {
    self.genex.db.collection('aposDocs', function(err, collection) {
      self.db = collection;
      return callback(err);
    });
  };
  ...
  self.ensureIndexes = function(callback) {
    ...
    function indexText(callback) {
      return self.ensureTextIndex(function(err) {
        if (err) {
          self.genex.utils.error('WARNING: unable to ensure text index, apostrophe:migrate can fix that');
        }
  ...
  // cursor methods:
  //
  // genex.docs.find(req, { type: 'foobar' }).toArray(
  //   function(err, docs) { ... }
  // );
  ...
  self.find = function(req, criteria, projection) {
    ...
    return self.genex.create('genesys-cursor', { genex: self.genex, req: req, criteria: criteria, projection: projection });
  };
  ...
  // genex.*.docFixUniqueError is called with the
  ...
  self.deleteFromTrash = function(req, idOrCriteria, callback) {
    ...
    criteria.trash = true;
    criteria = {
      $and: [ criteria, self.genex.permissions.criteria(req, 'publish-doc') ]
    };
  ...
  self.docBeforeInsert = function(req, doc, options) {
    ...
    if (!doc._id) {
      doc._id = self.genex.utils.generateId();
    }
  ...
  self.testInsertPermissions = function(req, doc, options) {
    ...
    if (!(options.permissions === false)) {
      if (!self.genex.permissions.can(req, 'edit-' + doc.type, null, doc)) {
  ...
  self.docBeforeSave = function(req, doc, options) {
    ...
    _.each(manager.schema, function(field) {
      if (field.sortify) {
        doc[field.name + 'Sortified'] = self.genex.utils.sortify(doc[field.name] ? doc[field.name] : '');
      }
    });
  ...
  self.ensureSlug = function(doc) {
    ...
    if ((!doc.slug) || (doc.slug === 'none')) {
      if (doc.title) {
        doc.slug = self.genex.utils.slugify(doc.title);
  ...
  self.docBeforeUpdate = function(req, doc, options) {
    ...
    if (options.permissions !== false) {
      if (!self.genex.permissions.can(req, 'edit-' + doc.type, doc)) {
        throw new Error('forbidden');
      }
    }
  };
  ...
  //
  // self.genex.docs.getManager(doc.type).update(...)
  //
  ...
  self.updateBody = function(req, doc, options, callback) {
    ...
    return self.retryUntilUnique(req, doc, function(callback) {
      return self.db.update({ _id: doc._id }, self.genex.utils.clonePermanent(doc), callback);
    }, callback);
  };
  ...
  self.insertBody = function(req, doc, options, callback) {
    ...
    return self.retryUntilUnique(req, doc, function(callback) {
      return self.db.insert(self.genex.utils.clonePermanent(doc), callback);
    }, callback);
  };
  ...
  // is a module that subclasses `genesys-doc-type-manager`
  // (or its subclasses `genesys-pieces` and `genesys-custom-pages`).
  ...
  // with `genesys-pages` always have one).
  ...
  self.lock = function(req, id, contextId, options, callback) {
    ...
    criteria = {
      $and: [
        criteria,
        self.genex.permissions.criteria(req, 'edit')
      ]
    };
      ...
      if (!result.result.nModified) {
        return self.db.findOne({ _id: id }, { advisoryLock: 1 }, function(err, info) {
          ...
          if (info.advisoryLock.username === req.user.username) {
            return callback('locked-by-me', self.genex.i18n.__('You may be editing that document in another tab or window.\nIf you take control, you could lose work in that tab or window.\nDo you want to take control?'));
          }
          return callback('locked', self.genex.i18n.__(
        ...
        function(err, info) {
          ...
          if ((!info.advisoryLock) || (info.advisoryLock._id !== contextId)) {
            if (info.advisoryLock && info.advisoryLock.username === req.user.username) {
              message = self.genex.i18n.__('You are now editing this document in another tab or window.');
              return callback('locked-by-me');
            }
            message = self.genex.i18n.__('Control of this document has been taken by another user.');
