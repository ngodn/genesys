...
genex.define('apostrophe-polymorphic-manager-manager-modal', {
  ...
  construct: function(self, options) {
    ...
    self.enableChooserViews = function() {
      _.each(self.options.chooser.field.withType, function(type) {
        var view = genex.docs.getManager(type).getTool('manager-modal', {
      ...
      self.$el.find('[data-tab-button]').removeClass('genex-active');
      self.$el.find('[data-tab-button]:first').addClass('genex-active');
      self.$el.on('click', '[data-tab-button]', function() {
        var tabName = $(this).attr('data-tab-button');
        self.$el.find('[data-tab-button]').removeClass('genex-active');
        $(this).addClass('genex-active');

        self.$el.find('[data-tab]').removeClass('genex-chooser-tab--active');
        self.$el.find('[data-tab="' + tabName + '"]').addClass('genex-chooser-tab--active');
        return false;
      });
      self.hideOthers();
    };
    ...
    self.enableChooser = function(callback) {
      return self.parentChooser.clone(
        ...
        function(err, chooser) {
          ...
          genex.on('pieceInserted', self.chooser.pieceInsertedListener);
          return callback(null);
        }
      );
    };
    ...
    self.afterHide = function() {
      genex.off('pieceInserted', self.chooser.pieceInsertedListener);
    ...
    self.decorateManager = function(manager, options) {
      ...
      manager.beforeShow = function(callback) {
        manager.$el.find('.genex-chooser').remove();
