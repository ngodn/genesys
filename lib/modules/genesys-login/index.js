...
// the `genesys-passport` module, which adds support for login via
...
// The default is `48`.
//
// ## Notable properties of genex.modules['genesys-login']
//
// `passport`
//
...
module.exports = {
  ...
  singletonWarningIfNot: 'genesys-login',
  ...
  construct: function(self, options) {
    ...
    // user via the [genesys-users](https://docs.apostrophecms.org/apostrophe/modules/genesys-users)
    ...
    self.deserializeUser = function(id, callback) {
      ...
      return self.genex.users.find(req, { _id: id }).toObject(function(err, user) {
        if (err) {
          return callback(err);
        }
        if (!user) {
    ...
    // [genesys-users](https://docs.apostrophecms.org/apostrophe/modules/genesys-users) module.
    // Users with the `disabled` property set to true may not log in.
    // Passwords are verified via the `verifyPassword` method of
    // [genesys-users](https://docs.apostrophecms.org/apostrophe/modules/genesys-users), which is
    ...
    self.verifyLogin = function(username, password, callback) {
      var req = self.genex.tasks.getReq();
      return self.genex.users.find(req, {
        $or: [
          { username: username },
          { email: username }
        ],
        ...
        return self.genex.users.verifyPassword(user, password, function(err) {
          if (err) {
            // Slow down and keep 'em hanging to make brute force attacks less easy
            return setTimeout(function () {
              return callback(null, false);
    ...
    self.addRoutes = function() {
      if (self.options.localLogin !== false) {
        self.genex.app.get('/login', function(req, res) {
          if (req.user) {
      ...
      if ((self.options.localLogin !== false) && self.options.passwordReset) {
        ...
        self.genex.app.get('/password-reset-request', function(req, res) {
          if (req.user) {
            // User is already logged in, redirect to home page
            return res.redirect('/');
          }
        ...
        self.genex.app.post('/password-reset-request', function(req, res) {
        ...
        self.genex.app.get('/password-reset', function(req, res) {
          var reset = self.genex.launder.string(req.query.reset);
          var email = self.genex.launder.string(req.query.email);
          if (!reset.length) {
            return res.redirect('/password-reset-request?error=missing');
          }
          req.scene = 'user';
          var adminReq = self.genex.tasks.getReq();
          return self.genex.users.find(adminReq, {
            email: email,
        ...
        self.genex.app.post('/password-reset', function(req, res) {
          var reset = self.genex.launder.string(req.body.reset);
          var email = self.genex.launder.string(req.body.email);
          var password = self.genex.launder.string(req.body.password);
          ...
          return Promise.try(function() {
            ...
            return self.genex.users.verifySecret(user, 'passwordReset', reset).then(function() {
              return user;
            });
          }).then(function(user) {
            user.password = password;
            delete user.passwordResetAt;
            return self.genex.users.update(adminReq, user);
          }).then(function(user) {
            return self.genex.users.forgetSecret(user, 'passwordReset');
          }).then(function(user) {
      ...
      self.genex.app.get('/logout', function(req, res) {
    ...
    self.sendPasswordResetEmail = function(req, user) {
      ...
      var reset;
      return Promise.try(function() {
        reset = self.genex.utils.generateId();
        user.passwordReset = reset;
        user.passwordResetAt = new Date();
        return self.genex.users.update(req, user, { permissions: false }).then(function() {
          return user;
        });
      }).then(function(user) {
        if (!user.email) {
    ...
    self.addAdminBarItems = function() {
      self.genex.adminBar.add(self.__meta.name + '-logout', 'Log Out', null, { last: true, href: '/logout' });
    };
    ...
    self.modulesReady = function() {
      ...
      self.genex.users.addSecret('passwordReset');
    };
