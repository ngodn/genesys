// Provides genex.ui, a singleton that provides user interface features
// used throughout Apostrophe on the browser side.
//
// ## Properties of interest
//
// ### genex.ui.shiftActive
//
// True whenever the shift key is down.

genex.define('genesys-ui', {
  ...
  construct: function(self, options) {
    ...
    // the data-genex-tooltip="YR TOOLTIP MSG" pattern
    ...
    self.prepTooltip = function() {
      var $tooltip = $('[data-genex-tooltip-template]');
      var $clone = $tooltip.clone();
      return $clone
        .removeAttr('data-genex-tooltip-template')
        .attr('data-genex-tooltip', 'true')
        .removeClass('genex-hidden');
    };
    ...
    self.createTooltip = function($el) {
      ...
      $newTooltip.text($this.attr('data-genex-tooltip'));
      ...
      if (top < 0) {
        $newTooltip.addClass('genex-tooltip--bottom');
      ...
      $newTooltip.addClass('genex-tooltip--visible');
    ...
    self.enableShift = function() {
      ...
      $body.keyup(function(e) {
        if (e.keyCode === 16) {
          self.shiftActive = false;
          genex.emit('shiftUp', e);
        }
      });
    };

    // Utility that lets you add the data attribute 'data-genex-actionable' to any element and have
    // its `genex-active` class be toggled on click.
    //
    ...
    self.enableActionable = function() {
      var $body = $('body');
      $body.on('click', '[data-genex-actionable]', function(event) {
        var $el = $(this);
        if ($el.attr('data-genex-actionable')) {
          $el.closest('[' + $el.attr('data-genex-actionable') + ']').toggleClass('genex-active');
        } else {
          $el.toggleClass('genex-active');
        }
      });
    };

    // Implement automatic width adjustment for `[data-genex-dropdown]` menus that have a
    // `[data-genex-dropdown-button-label]`.

    self.enhanceDropdowns = function() {
      $('body').on('click change', '[data-genex-dropdown]', function(event) {
        event.preventDefault();
        var $this = $(this);
        if ($this.find('[data-genex-dropdown-button-label]').length) {
          // only adjust width for button'y dropdowns
          // not global menu dropdowns
          var $label = $(this).find('[data-genex-dropdown-button-label]');
          if ($label.attr('data-genex-dropdown-button-label') === 'active') {
            $label.css('padding-right', $label.attr('data-genex-original-right-padding'));
            $label.attr('data-genex-dropdown-button-label', '');
          } else {
            $label.attr('data-genex-original-right-padding', $label.css('padding-right'));
            var val = parseInt($label.attr('data-genex-original-right-padding').substring(0, $label.attr('data-genex-original-right-padding').length - 2));
            var extra = ($(this).find('[data-genex-dropdown-items]')[0].getBoundingClientRect().width - $label[0].getBoundingClientRect().width + val);
            $label.css('padding-right', extra);
            $label.attr('data-genex-dropdown-button-label', 'active');
          }
        }
      });
    };
    ...
    // See also [genesys-express](https://docs.apostrophecms.org/apostrophe/modules/genesys-express)
    ...
    // locks are released.
    //
    // See also genex.busy for interactions that
    ...
    self.globalLock = function() {
      ...
      var $freezer = $('<div class="genex-global-busy"></div>');
      $('body').append($freezer);
    ...
    self.enhanceColorpicker = function($el, options) {
      ...
      function emptyColorpicker() {
        $label.text($el.attr('data-genex-color-empty-label'));
        $preview.addClass('genex-field-input-color-preview--empty');
      ...
      function populateColorpicker() {
        if ($el.val()) {
          ...
          $preview.removeClass('genex-field-input-color-preview--empty');
          $preview.css('background-color', $el.spectrum('get').toHexString());
        }
      }
    };

    // Converts genesys-schemas 24 hour time field strings
    ...
    // of the attribute.
    //
    // self.link('genex-manage', 'blogPost', fn)
    //
    ...
    // flow that way, for example 'genex-workflow-export' does not
    ...
    self.link = function(sel, verb, object, callback) {
      ...
      var attribute;
      if (object === null) {
        attribute = 'data-' + genex.utils.cssName(verb);
      } else {
        attribute = 'data-' + genex.utils.cssName(verb) + '-' + genex.utils.cssName(object);
      }
    ...
    self.enableAjax = function() {
      $('body').on('submit', '[data-genex-ajax-context] form', self.ajaxSubmitHandler);
      $('body').on('click', '[data-genex-ajax-context] a', self.ajaxClickHandler);
    ...
    self.ajaxInfiniteScrollHandler = function() {
      var $infiniteScroll = $('[data-genex-ajax-infinite-scroll]');
    ...
    self.ajaxSubmitHandler = function(event) {
      ...
      // (window.location.pathname.replace(new RegExp('^' + genex.utils.regExpQuote(genex.prefix)), ''));
    ...
    self.ajaxClickHandler = function(event) {
      ...
      if ($anchor.attr('data-genex-no-ajax')) {
        return true;
      }
    ...
    // (the `data-genex-ajax-context` attribute of its closest ancestor that has one).

    self.ajaxContextOf = function($el) {
      return $el.closest('[data-genex-ajax-context]').attr('data-genex-ajax-context');
    };

    // Refresh the named `data-genex-ajax-context` with the content returned by the specified
    ...
    // is explicitly `false`.
    //
    // Any text input or textarea fields, and any other elements with a distinct data-genex-ajax-preserve
    ...
    // `data-genex-ajax-preserve` and `data-genex-ajax-preserved` attributes.
    //
    // If `url` or `data` contains an `append` parameter set to `1`, then the portion of
    // the new content that falls within an element with the `data-genex-ajax-append` attribute
    // is appended to the corresponding element already nested within `data-genex-ajax-content`.
    // However everything *not* in side the `data-genex-ajax-append` element is replaced in the
    ...
    self.ajaxGo = function(name, url, data, options) {
      data = data || {};
      var $old = $('[data-genex-ajax-context="' + name + '"]');
      var $appendTo = $old.find('[data-genex-ajax-append]');
      var append = false;
      ...
      // add an genex-ajax=1 parameter if not present

      if (url.match(/[?&]genex-ajax=1/)) {
        hasAposAjax = true;
      }
      ...
      $.ajax({
        ...
        success: function(html) {
          ...
          if (append) {
            ...
            $appending = $new.find('[data-genex-ajax-append]');
          ...
          // a concern you can avoid it by checking for `data-genex-ajax-preserved`
          genex.emit('enhance', $old);
          genex.emit('ajax', $old);
          if (append) {
    ...
    // Push the specified URL to the browser history, associating it with the named data-genex-ajax-context
    // element in the page. Any genex-ajax query parameter is stripped off so that the history shows
    ...
    // within `data-genex-ajax-context`. To avoid unexpected results the default behavior
    ...
    self.ajaxError = function(err) {
      ...
      genex.notify('Unfortunately a server error occurred. Please try again later.', { type: 'error', dismiss: true });
    };
    ...
    // of $old with a data-genex-ajax-preserve attribute are preserved, and of
    ...
    //
    // Each preserved element must have a **unique value** for data-genex-ajax-preserve,
    ...
    self.replaceUnpreserved = function($old, $new, options) {
      ...
      function tagAncestors($context) {
        ...
        find($context, 'preserves').removeAttr('data-genex-ajax-preserves');
      ...
      function find($el, key) {
        return $el.find('[data-genex-ajax-' + key + ']');
      }

      // Find prefixed with specified value
      function findVal($el, key, val) {
        return $el.find('[data-genex-ajax-' + key + '="' + val + '"]');
      }

      // .is() prefixed
      function is($el, key) {
        return $el.is('[data-genex-ajax-' + key + ']');
      }

      // is() prefixed with specified value
      function isVal($el, key, val) {
        return $el.is('[data-genex-ajax-' + key + '="' + val + '"]');
      }

      // .attr() prefixed, getter and setter
      function attr($el, key, val) {
        if (arguments.length === 2) {
          return $el.attr('data-genex-ajax-' + key);
        }
        $el.attr('data-genex-ajax-' + key, val);
      }
