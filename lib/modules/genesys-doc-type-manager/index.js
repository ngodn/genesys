// This module is the base class of `genesys-custom-pages`, `genesys-pieces`,
// `genesys-global` and any other module that serves as the manager for a
...
module.exports = {
  ...
  afterConstruct: function(self) {
    ...
    self.defineCursor();
    self.composeSchema();
    self.genex.docs.setManager(self.name, self);
    self.pushAssets();
  ...
  beforeConstruct: function(self, options) {
    ...
    options.addFields = [
      ...
      },
      {
        ...
        contextual: !options.genex.docs.trashInSchema,
        def: false
      }
    ...
    options.arrangeFields = [
      {
        name: 'basics',
        label: 'Basics',
        fields: [ 'title', 'slug', 'published', 'tags' ].concat(options.genex.docs.trashInSchema ? [ 'trash' ] : [])
      },
      {
        name: 'permissions',
        label: 'Permissions',
        fields: options.genex.schemas.getPermissionsFieldNames(),
        last: true
      }
    ].concat(options.arrangeFields || []);
    if (options.genex.docs.trashInSchema) {
  ...
  construct: function(self, options) {
    ...
    if (!options.name) {
      throw new Error('genesys-doc-type-manager requires name option in module ' + self.__meta.name);
    }
