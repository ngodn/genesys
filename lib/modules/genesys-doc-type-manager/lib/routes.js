...
module.exports = function(self, options) {
  ...
  self.route('post', 'chooser-choices', function(req, res) {
    ...
    return async.series({
      convert: function(callback) {
        ...
        return self.genex.schemas.convert(req, [ _.omit(field, 'readOnly') ], 'form', input, receptacle, callback);
      },
      join: function(callback) {
        return self.genex.schemas.join(req, [ field ], receptacle, true, callback);
      }
    }, function(err) {
      if (err) {
  ...
  self.renderRoute('post', 'relationship-editor', function(req, res, next) {
    var field = req.body.field;
    if (!self.genex.utils.isBlessed(req, field, 'join')) {
  ...
  self.apiRoute('post', 'autocomplete', function(req, res, next) {
    ...
    var term = self.genex.launder.string(req.body.term);
