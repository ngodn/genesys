...
module.exports = function(self, options) {
  ...
  //
  // For suffixes, genesys-pages will take care of adding and removing
  ...
  self.deduplicateTrash = function(req, doc, callback) {
    ...
    return self.genex.docs.db.update({ _id: doc._id }, { $set: $set }, callback);
  };
  ...
  self.deduplicateRescue = function(req, doc, callback) {
    ...
    function update(callback) {
      ...
      return self.genex.docs.db.update({ _id: doc._id }, action, callback);
    }
    function checkOne(name, callback) {
      ...
      return self.genex.docs.db.findOne(query, { _id: 1 }, function(err, found) {
        if (err) {
          return callback(err);
        }
        if (found) {
  ...
  self.newInstance = function() {
    var doc = self.genex.schemas.newInstance(self.schema);
    doc.type = self.name;
    return doc;
  };
  ...
  // Used by `genesys-versions` to label changes that
  ...
  self.allowedSchema = function(req) {
    ...
    if (typeIndex !== -1) {
      ...
      // genesys-option-overrides and genesys-workflow modules,
      // if present, can be used together to disable various types based
      // on locale settings
      disabled = self.genex.pages.getOption(req, 'disabledTypes');
      if (disabled) {
  ...
  self.composeSchema = function() {
    ...
    if (self.isAdminOnly()) {
      options.removeFields = (options.removeFields || []).concat(self.genex.schemas.getPermissionsFieldNames());
    }

    self.schema = self.genex.schemas.compose(self.options, self);
    ...
    if (slug) {
      ...
      if (self.genex.docs.options.deconflictSlugs === false) {
  ...
  self.autocomplete = function(req, query, callback) {
    ...
    if (query.term && query.term.toString && query.term.toString().length) {
      var term = self.genex.launder.string(query.term);
    ...
    cursor.toArray(function(err, docs) {
      ...
      if (query.values) {
        docs = self.genex.utils.orderById(query.values, docs);
      }
  ...
  self.addSearchIndexListener = function() {
    self.genex.on('docSearchIndex', self.searchIndexListener);
  };
  ...
  self.searchIndexListener = function(doc, texts) {
    ...
    self.genex.schemas.indexFields(self.schema, doc, texts);
  };
  ...
  // `genex.utils.sortify` so that it sorts and compares in a more
  ...
  self.addSortifyMigration = function(field) {
    ...
    return self.genex.migrations.addSortify(self.__meta.name, { type: self.name }, field);
  };
  ...
  self.addSlugPrefixMigration = function() {
    ...
    self.genex.migrations.add(self.__meta.name + 'SlugPrefix', function() {
      return self.genex.migrations.eachDoc({
        type: self.name,
        slug: {
          $not: new RegExp('^' + self.genex.utils.regExpQuote(self.options.slugPrefix))
        }
      }, function(doc) {
  ...
  self.getBatchPermissionsSchema = function(req) {
    return self.genex.schemas.subset(self.allowedSchema(req), self.genex.schemas.getPermissionsFieldNames());
  };
