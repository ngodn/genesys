genex.define('genesys-doc-type-manager-chooser', {
  ...
  construct: function(self, options) {
    ...
    self.refresh = function(options) {
      ...
      return self.api('chooser-choices', { choices: self.choices, field: self.field, validate: true }, function(result) {
        ...
        return async.eachSeries(self.choices || [], function(choice, callback) {
          ...
          return genex.schemas.populate(self.$choices.find('[data-relationship-inline="' + choice.value + '"]'), self.inlineSchema, choice, callback);
        }, function(err) {
          if (err) {
            // Ouch
            genex.notify('Unable to populate form correctly, try again later', { type: 'error', dismiss: true });
            self.decrementRefreshing();
            return;
          }
    ...
    self.convertInlineRelationships = function(callback) {
      return async.eachSeries(self.choices, function(choice, callback) {
        ...
        return genex.schemas.convert($relationship, self.inlineSchema, choice, callback);
      }, callback);
    };
    ...
    self.enableLinks = function() {
      self.$el.on('mouseover', '[data-genex-tooltip]', genex.ui.createTooltip);
      self.$el.on('mouseleave click', '[data-genex-tooltip]', genex.ui.removeTooltip);
      self.link('genex-delete', 'item', function($button, _id) {
        var index = _.findIndex(self.choices, { value: _id });
        if (index === -1) {
          return;
        }
        self.remove(_id, false);
        var $choices = self.$el.find('[data-choices]>.genex-chooser-choice');
      ...
      self.link('genex-raise', 'item', function($button, _id) {
        var index = _.findIndex(self.choices, { value: _id });
        if (index === -1) {
          return;
        }
      ...
      self.link('genex-lower', 'item', function($button, _id) {
        var index = _.findIndex(self.choices, { value: _id });
        if (index === -1) {
          return;
        }
      ...
      self.link('genex-relate', 'item', function($button, _id) {
      ...
      self.link('genex-edit', 'item', function($button, _id) {
        self.manager.edit(_id, function() {});
      });
      self.link('genex-browse', function() {
        self.launchBrowser();
      });
    };
    ...
    self.enableSortable = function () {
      var sortableOptions = {
        cancel: false,
        handle: '[data-genex-sort-item]',
        update: function(event, ui) {
          var $choices = self.$el.find('[data-chooser-choice]');

          if ($choices.length !== self.choices.length) {
            genex.notify('Something went wrong, reverting changes', { type: 'error' });
    ...
    self.clone = function(options, callback) {
      ...
      return genex.create(self.__meta.name, _options, function(err, chooser) {
        if (err) {
          return callback(err);
        }
    ...
    self.onChange = function() {
      if ((self.limit > 1) && self.choices.length >= self.limit) {
        self.$el.addClass('genex-chooser-full');
        if (self.options.managerModal) {
          self.options.managerModal.$el.addClass('genex-chooser-full');
        }
        self.$autocomplete.prop('disabled', true);
        self.full = true;
      } else {
        self.$el.removeClass('genex-chooser-full');
        self.$autocomplete.prop('disabled', false);
        if (self.options.managerModal) {
          self.options.managerModal.$el.removeClass('genex-chooser-full');
        }
        self.full = false;
      }
    ...
    // Adds and overrides methods of the genesys-pieces-manager-modal to
      ...
      manager.enableChooser = function(callback) {
        ...
        return manager.parentChooser.clone(
          ...
          function(err, chooser) {
            ...
            genex.on('pieceInserted', chooser.pieceInsertedListener);
            manager.afterEnableChooser();
            return callback(null);
          }
        );
    ...
    self.afterManagerSave = function() {
      genex.off('pieceInserted', self.pieceInsertedListener);
    };
