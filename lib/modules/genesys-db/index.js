...
// This module establishes `genex.db`, the mongodb driver connection object.
//
// ## Options
//
// ### `uri`
...
//
// Note that `genex.db` is the mongodb connection object, not this module.
// You shouldn't need to talk to this module after startup, but you can
// access it as `genex.modules['genesys-db']` if you wish.
//
// If you need to change the way MongoDB connections are made,
// override `connectToMongo` in `lib/modules/genesys-db/index.js`
// in your project.
  ...
  construct: function(self, options) {
    ...
    self.connectToMongo = function(callback) {
      if (self.options.db) {
        ...
        self.genex.db = self.options.db.db(options.name || self.genex.shortName);
      ...
      if (process.env.APOS_MONGODB_URI) {
        ...
        if (!options.name) {
          options.name = self.genex.shortName;
        }
      ...
      if (uri.match(/^mongodb\+srv/)) {
        return callback(new Error('You are attempting to use a mongodb+srv URI, which is only\n' +
          ...
          '2. Install the genesys-db-mongo-3-driver module which supports the new URI.\n'));
      }
      ...
      return mongo.MongoClient.connect(uri, connectOptions, function (err, dbArg) {
        self.genex.db = dbArg;
        if (err) {
          self.genex.utils.error('ERROR: There was an issue connecting to the database. Is it running?');
        }
        return callback(err);
      });
    ...
    self.keepalive = function() {
      self.keepaliveCollection = self.genex.db.collection('aposKeepalive');
    ...
    // `genesys-db:reset` task. Then Apostrophe carries out the usual
    ...
    self.earlyResetTask = function(callback) {
      ...
      // - Tom & Sam
      if (self.genex.argv._[0] === 'genesys-db:reset') {
    ...
    // because the `genesys-db-mongo-3-driver` module provides the
    ...
    self.bcPatch = function() {
      ...
      var prototype = Object.getPrototypeOf(self.genex.db.collection('aposKeepalive'));
      prototype.findWithProjection = prototype.find;
    };

    self.genex.tasks.add('genesys-db', 'reset',
      'Usage: node app genesys-db:reset\n\n' +
    ...
    self.dropAllCollections = function(callback) {
      return self.genex.db.collections(function(err, collections) {
        if (err) {
          return callback(err);
        }
    ...
    // interval timer. Sets `genex.db.closed` to true,
    ...
    self.apostropheDestroy = function(callback) {
      ...
      return self.genex.db.close(false, function(err) {
        if (err) {
          return callback(err);
        }
        self.genex.db.closed = true;
        return callback(null);
      });
    };

    self.trace = function() {
      var superCollection = self.genex.db.collection;
      self.genex.db.collection = function(name, options, callback) {
        if (callback) {
          return superCollection.call(self.genex.db, name, options, function(err, collection) {
            if (err) {
              return callback(err);
            }
            decorate(collection);
          ...
          var collection = superCollection.apply(self.genex.db, arguments);
          decorate(collection);
          return collection;
        }
