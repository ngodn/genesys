...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.ensureIndexes = function(callback) {
      return self.genex.docs.db.ensureIndex({ historicUrls: 1 }, callback);
    };

    self.pageNotFound = function(req, callback) {
      return self.genex.docs.find(req, { historicUrls: { $in: [ req.url ] } }).sort({ updatedAt: -1 }).toObject(function(err, doc) {
        if (err) {
          return callback(err);
        }
        if (!doc) {
    ...
    self.pageBeforeSend = function(req, callback) {
      ...
      return async.eachSeries(docs, function(doc, callback) {
        return self.genex.docs.db.update({ _id: doc._id }, {
          $addToSet: {
