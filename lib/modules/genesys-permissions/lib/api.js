...
module.exports = function(self, options) {
  ...
  // allow file uploads by anonymous users for genesys-moderator.
  //
  ...
  self._check = function(req, action, event, _true, _false, object, newObject, then) {
    ...
    function filter(response) {
      ...
      self.genex.emit(event, req, action, object, info);
  ...
  // the generic `genesys-page`, because pages can change type.
  ...
  self.getEffectiveTypeName = function(type) {
    ...
    if (self.genex.instanceOf(manager, 'genesys-custom-pages')) {
      return 'genesys-page';
    }
    return type;
  };
  ...
  //
  // Used internally to implement self.genex.permissions.criteria().
