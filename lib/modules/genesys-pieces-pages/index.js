// `genesys-pieces-pages` implements "index pages" that display pieces of a
...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.pieces = self.genex.modules[self.piecesModuleName];
    self.piecesCssName = self.genex.utils.cssName(self.pieces.name);
    ...
    // `genesys-pieces-cursor` in your subclass of
    // `genesys-pieces`.
    ...
    self.indexPage = function(req, callback) {
      ...
      if (self.pieces.contextual) {
        ...
        }).concat(self.genex.pages.options.contextMenu);
    ...
    // invoking `self.showPage`. See [genesys-custom-pages](https://docs.apostrophecms.org/apostrophe/modules/genesys-custom-pages)
    ...
    self.buildUrl = function(page, piece) {
      ...
      return self.genex.utils.addSlashIfNeeded(page._url) + piece.slug;
    };

    // Make the browser-side `genex` object aware of the current
    // in-context piece, as `genex.contextPiece`. Just enough to
    ...
    self.pushContextPiece = function(req) {
      if (self.pieces.options.contextual && req.data.piece) {
        req.browserCall('genex.contextPiece = ?', _.pick(req.data.piece, '_id', 'title', 'slug', 'type'));
      }
    };

    var superPageBeforeSend = self.pageBeforeSend;

    // Calls `pushContextPiece` to make `genex.contextPiece` available
    // in the browser
    ...
    // Aliased as the `addUrls` method of [genesys-pieces](https://docs.apostrophecms.org/apostrophe/modules/genesys-pieces), which
    // is invoked by the `addUrls` filter of [genesys-cursor](https://docs.apostrophecms.org/apostrophe/modules/genesys-docs/server-genesys-cursor).
