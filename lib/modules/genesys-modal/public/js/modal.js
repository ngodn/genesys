// `genesys-modal` is a base class for modal dialog boxes. Define a new
// type that extends `genesys-modal`, set the `source` option to the
...
// *Examples*
//
// The `genesys-tags`, `genesys-widgets` and `genesys-pieces` modules
...
// on the stack has the `genex-modal-slideable` CSS class, in which case
// the new modal "slides in" creating a breadcrumb trail.
//
// Subclasses of `genesys-modal` can also provide a `$view` jQuery
...
genex.define('genesys-modal', {
  ...
  construct: function(self, options) {
    ...
    self.enableGlobalEventsOnce = function() {
      if (!genex.modalSupport.initialized) {
        genex.modalSupport.initialized = true;
        ...
        $(document).on({
          'keydown.aposModal': function(e) {
            if (e.keyCode === 27) {
              genex.modalSupport.cancelTopModal();
              return false;
            }
          },
          'click.aposModal': function(e) {
            if (e.target.className === 'genex-modal-blackout') {
              genex.modalSupport.cancelTopModal();
              return false;
            }
          }
    ...
    self.enableButtonEvents = function() {
      ...
      self.$controls.on('click', '[data-genex-cancel]', function() {
        return self.cancel();
      });

      self.$controls.on('click', '[data-genex-save]', function() {
        var $button = $(this);
    ...
    self.enableBreadcrumbEvents = function() {
      self.$el.on('click', '[data-modal-breadcrumb]:first .genex-modal-breadcrumb-item', function() {
        var $item = $(this);
    ...
    // or there is no modal already open with the `genex-modal-slideable`
    ...
    // the `genex-modal-slideable` CSS class and, if so, returns
    // a jQuery reference to that modal.

    self.getSlideableAncestorEl = function() {
      var stack = genex.modalSupport.stack;
      if (!stack.length) {
        return false;
      }
      var $slideableAncestorEl = stack[stack.length - 1];

      if (!$slideableAncestorEl.hasClass('genex-modal-slideable')) {
        return false;
      }
    ...
    // Returns the `genesys-modal` object, not a jQuery element. Use findSafe
    ...
    // Returns the `genesys-modal` objects corresponding to each
    // slide nested in this modal, which presumably is a slide parent
    // modal (one with the `genex-modal-slideable` CSS class). The
    ...
    // the top modal already on the stack has the `genex-modal-slideable`
    ...
    // modal on the stack has the `genex-modal-slideable` CSS class.
    ...
    self.slideIn = function() {
      ...
      setImmediate(function() {
        $wrapper.find('[data-modal-content]').removeClass('genex-modal-slide-current');
        var fired = false;
        ...
        $content.addClass('genex-modal-slide-current');
      });

    };

    // Make the current modal's depth available as an attribute on various
    // elements such as `data-genex-modal-instructions`. This is needed for
    // reliable Nightwatch testing
    self.setDepthAttribute = function($el) {
      $el.attr('data-genex-modal-depth', genex.modalSupport.depth);
    };
    ...
    // the stack or the parent modal does not have the `genex-modal-slideable`
    // CSS class.
    ...
    self.stackPush = function() {
      ...
      genex.modalSupport.stack.push(self.$el);

      genex.emit('modalStackPush');

      self.$el.addClass('genex-modal-stack-push');
      $('body').append(self.$el);

      self.$el.show();
      self.$el.removeClass('genex-modal-stack-push');
    ...
    // all have the data-genex-modal-current attribute, and
    // that nothing else does.

    self.indicateCurrentModal = function(adding) {
      if (adding) {
        genex.modalSupport.all.push(self);
      } else {
        var top = genex.modalSupport.getLatestModal();
        if (top !== self) {
          genex.utils.error('Types do not match in indicateCurrentModal');
        }
        genex.modalSupport.all.pop();
      }
      $('body [data-genex-modal-current]').removeAttr('data-genex-modal-current');
      var latest = genex.modalSupport.getLatestModal();
      if (latest) {
        _.each([ latest.$el, latest.$controls, latest.$modalFilters, latest.$instructions ], function($el) {
          $el.attr('data-genex-modal-current', latest.__meta.name);
        });
      }
    };
    ...
    self.resizeContentHeight = function() {
      ...
      var headerHeight = $container.find('.genex-modal-header').outerHeight() || 0;
      var breadcrumbHeight = $container.find('.genex-modal-breadcrumb').outerHeight() || 0;
      var footerHeight = $container.find('.genex-modal-footer').outerHeight() || 0;

      var contentHeight = 'calc(100vh - 50px - ' + (headerHeight + breadcrumbHeight) + 'px)';

      // Set height on .genex-modal-content.
      // Check if self.$el is a sliding .genex-modal-content or modal wrapper

      var $content;
      if (self.$el.is('.genex-modal-content')) {
        $content = self.$el;
        ...
        $content = self.$el.find('.genex-modal-content').last();

        // dont resize the height of modals within modals
        if ($content.parents('[data-tab]').length === 0) {
          $content.css('height', contentHeight);
          self.$el.find('.genex-modal-content').find('[data-schema-tabs]').css('height', contentHeight);
        }
      }

      // genex-modal-footer is now *inside* genex-modal-content, we need to
      // adjust the height of genex-modal-body relative to it. -Tom
      var $body = $content.find('.genex-modal-body');

      if ($body.parents('[data-tab]').length === 0) {
        $body.css('height', 'calc(100% - ' + footerHeight + 'px)');
      }

      // genex-chooser needs the same treatment, although the CSS seems to imply it should not.
      var $chooser = $content.find('.genex-chooser');
      if ($chooser.length) {
    ...
    // schema field (see `genesys-schemas`) or a modal related to editing one
    ...
    self.refreshBreadcrumb = function() {
      ...
      if (!$ancestor) {
        if (self.$el.hasClass('genex-modal-slideable')) {
          $ancestor = self.$el;
        }
        if (!$ancestor) {
          return;
      ...
      _.each(slides, function(slide, i) {
        var $li = $('<li></li>');
        $li.addClass('genex-modal-breadcrumb-item');
    ...
    // such as `genesys-pieces-editor-modal` that provides its own version,
    ...
    self.stackPop = function() {
      ...
      var $current = genex.modalSupport.getTopModalOrBody();
      if ($current.data('aposSavedScrollTop') !== undefined) {
        $(window).scrollTop($current.data('aposSavedScrollTop'));
      }

      genex.modalSupport.stack.pop();

      genex.emit('modalStackPop');

      var $blackoutContext = genex.modalSupport.getTopModalOrBody();
      var $blackout = $blackoutContext.find('.genex-modal-blackout');
      if ($blackout.data('interval')) {
        clearInterval($blackout.data('interval'));
      }

      $blackout.addClass('genex-modal-blackout-fade');
      self.$el.addClass('genex-modal-stack-push');
      ...
      self.$el.one('webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend', function() {
        ...
        self.$body.removeClass('genex-active-blackout');

        if (!genex.modalSupport.stack.length) {
    ...
    // Applies a blackout div with the `genex-modal-blackout` CSS class to
    ...
    self.applyBlackout = function() {
      ...
      var $blackoutContext = genex.modalSupport.getTopModalOrBody();
      var $blackout = $('<div class="genex-modal-blackout"></div>');
      ...
      self.$body.addClass('genex-active-blackout');
    };
    ...
    self.focusFirstFormElement = function() {
      ...
      var $firstElm = self.$el.find('.genex-modal-body form fieldset:first input');
...
// `genex.modalSupport` provides a handful of global methods used to manipulate
...
genex.modalSupport = {
  ...
  // The objects are actual modal objects (`genesys-modal`),
  // NOT jQuery objects
  all: [],
  // Returns jQuery object
  getTopModalOrBody: function() {
    return $(genex.modalSupport.stack.length ? genex.modalSupport.stack[genex.modalSupport.stack.length - 1] : 'body');
  },
  // Returns actual genesys-modal object, including latest slide (not views)
  getLatestModal: function() {
    if (!genex.modalSupport.all.length) {
      return null;
    }
    return genex.modalSupport.all[genex.modalSupport.all.length - 1];
  },
  closeTopModal: function() {
    var topModal = genex.modalSupport.getTopModalOrBody();
