...
module.exports = function(self, options) {
  ...
  self.emit = function(name /* , arg1, arg2... */) {
    ...
    var eh = self.genex.eventHandlers[self.__meta.name] || {};
    return Promise.mapSeries(eh[name] || [], function(h) {
      var module = self.genex.modules[h.moduleName];
  ...
  self.on = function(name, methodName, fn) {
    ...
    self.genex.eventHandlers[moduleName] = self.genex.eventHandlers[moduleName] || {};
    var eh = self.genex.eventHandlers[moduleName];
    if (fn) {
      self[methodName] = fn;
    }
  ...
  self.callAllAndEmit = function(callAllName, eventName, /* argument, ... */ callback) {
    ...
    return self.genex.callAll.apply(self.genex, args);
  };
};
