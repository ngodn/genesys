...
// an implementation provided in another module, such as `genesys-assets`,
...
module.exports = {
  ...
  beforeConstruct: function(self, options) {
    ...
    self.genex = options.genex;
    // all apostrophe modules are properties of self.genex.modules.
    // Those with an alias are also properties of self.genex
    self.genex.modules[self.__meta.name] = self;
    if (self.options.alias) {
      if (_.has(self.genex, self.options.alias)) {
      ...
      self.genex[self.options.alias] = self;
    }
  },
  ...
  construct: function(self, options) {
    ...
    if (self.genex.assets) {
      if (!self.genex.assets.chains) {
        self.genex.assets.chains = {};
      }
      _.each(self.__meta.chain, function(meta, i) {
        self.genex.assets.chains[meta.name] = self.__meta.chain.slice(0, i + 1);
      });
    }
    ...
    // Add an Express route to genex.app. The `path` argument is
    ...
    //
    // `self.genex.app.post(self.action + '/edit-monkey', function(req, res) { ... })`
    //
    ...
    // `self.genex.middleware.files` for file uploads.
    //
    ...
    self.route = function(method, path, responder, fn) {
      var key = method + '.' + path;
      if (!routes[key]) {
        self.genex.app[method](self.action + '/' + path, function(req, res) {
          var args = routes[key];
    ...
    self.addHelpers = function(object /* or name, value */) {
      if (typeof (object) === 'string') {
        self.genex.templates.addHelpersForModule(self, arguments[0], arguments[1]);
      } else {
        self.genex.templates.addHelpersForModule(self, object);
      }
    };
    ...
    // only exceptions in apostrophe core are `genex.area` and `genex.singleton`.
    //
    // The helper must be added first with `addHelpers`.

    self.addHelperShortcut = function(name) {
      self.genex.templates.addHelperShortcutForModule(self, name);
    };
    ...
    self.pushAsset = function(type, name, options) {
      ...
      _.each(self.__meta.chain, function(typeMeta) {
        if (self.genex.assets.push(type, name, options, typeMeta)) {
          exists = true;
        }
      });
      if (!exists) {
        self.genex.utils.error('WARNING: no versions of the ' + type + ' ' + name + ' exist, but you are pushing that asset in the ' + self.__meta.name + ' module.');
      }
    };
    ...
    // can be accessed via the `genex` object.
    //
    ...
    self.render = function(req, name, data) {
      ...
      return self.genex.templates.renderForModule(req, name, data, self);
    };
    ...
    self.partial = function(name, data) {
      ...
      return self.genex.templates.partialForModule(name, data, self);
    };
    ...
    self.renderString = function(req, s, data) {
      ...
      return self.genex.templates.renderStringForModule(req, s, data, self);
    };
    ...
    self.partialString = function(req, s, data) {
      ...
      return self.genex.templates.partialStringForModule(s, data, self);
    };
    ...
    //
    // `genesys-templates:outerLayout.html`
    //
    // Or:
    //
    // `genesys-templates:refreshLayout.html`
    //
    ...
    self.renderPage = function(req, template, data) {
      return self.genex.templates.renderPageForModule(req, template, data, self);
    };
    ...
    self.sendPage = function(req, template, data) {
      return async.series([
        ...
        return req.res.send(
          self.genex.templates.renderPageForModule(req, template, data, self)
        );
      });
      ...
      function loadDeferredWidgets(callback) {
        return self.genex.areas.loadDeferredWidgets(req, callback);
      }
    ...
    // same alias on the `genex` object as it has on the server, if any,
    // otherwise it is still available via `genex.modules[full-module-name]`.
    //
    ...
    // even if an `genex.change` event would otherwise cause it to be created again.
    //
    ...
    self.pushCreateSingleton = function(req, when) {
      ...
      if (req) {
        req.browserCall('genex.createModule(?, ?, ?)', self.__meta.name, options, self.options.alias);
      } else {
        self.genex.push.browserCall(when, 'genex.createModule(?, ?, ?)', self.__meta.name, options, self.options.alias);
      }
    };
    ...
    // point. For instance, if `genesys-images` has already called this
    // as a subclass of `genesys-pieces`, then `genesys-files` will
    // just define its own cursor, extending `genesys-pieces-cursor`, and stop.
    // This prevents duplicate definitions when many types extend `genesys-pieces`.

    // If `options.stop` is set to the name of an ancestor module,
    // the chain stops **after** defining the related type for that module.

    // For instance, the module `genesys-files` extends
    // `genesys-pieces`, which extends `genesys-doc-type-manager`.

    // So when that module calls:

    // ```
    // self.defineRelatedType('cursor', {
    //   stop: 'genesys-doc-type-manager'
    // });
    // ```

    // We get:

    // genesys-files-cursor extends...
    // genesys-pieces-cursor which extends...
    // genesys-doc-type-manager-cursor which extends...
    // genesys-cursor (because `cursor.js` for doc-type-manager says so)
    ...
    self.defineRelatedType = function(tool, options) {
      ...
      // if (self.__meta.name === 'genesys-tags') {
      //   process.exit(1);
      // }
      function defineRelatedTypeBody(i, tool, options) {
        ...
        if (_.has(self.genex.synth.definitions, self.genex.synth.myToOriginal(name))) {
          // Already defined
          return;
        }
        ...
        if (inferred && ((extend === inferred) || (self.genex.synth.originalToMy(extend) === inferred))) {
          defineRelatedTypeBody(i - 1, tool, options);
        }
        if (self.genex.synth.isMy(name)) {
          // Restore the implicit subclassing
          name = self.genex.synth.myToOriginal(name);
        ...
        self.genex.synth.define(name, definition);

        function inferParentName() {
          if ((i > 0) && (chain[i].name !== options.stop)) {
            return self.genex.synth.myToOriginal(chain[i - 1].name + '-' + tool);
          } else {
            // A hard stop, so we don't implicitly extend
            // `genesys-module`. Related types are not modules
            return false;
          }
        }
    ...
    // See `defineRelatedType`. A convenient wrapper for calling `genex.create`.
    //
    // For instance, if this module is `genesys-images`, then
    // `self.createRelatedType('cursor', { options... })` will
    // create an instance of `genesys-images-cursor`.
    //
    ...
    self.createRelatedType = function(tool, options, callback) {
      return self.genex.synth.create(self.__meta.name + '-' + tool, options, callback);
    };

    // A convenience method to fetch properties of `self.options`.
    //
    // `req` is required to provide extensibility; modules such as
    // `genesys-workflow` and `genesys-option-overrides`
    ...
    self.logError = function(req, err) {
      if (!(req && req.res)) {
        self.genex.utils.error('Looks like you did not pass req to self.logError, you should not have to call this method yourself,\nit is usually called for you by self.apiRoute, self.htmlRoute or self.renderRoute', (new Error()).stack);
        return;
      }
      // err.stack also includes the description of the error
      self.genex.utils.error(req.method + ' ' + req.url + ': ' + '\n\n' + err.stack);
    };
    ...
    //
    // **The `nodemailer` option of the `genesys-email` module
    ...
    self.email = function(req, templateName, data, options, callback) {
      ...
      function body(callback) {
        return self.genex.modules['genesys-email'].emailForModule(req, templateName, data, options, self, callback);
      }
    };
    ...
    // the task.
    //
    // `fn` is invoked with `(genex, argv, callback)`. You may
    // return a promise, in which case you must *not* invoke `callback`.
    //
    // To carry out actions requiring `req` in your code, call
    // `self.genex.tasks.getReq` to get a `req` with unlimited admin permissions.

    self.addTask = function(name, usage, fn) {
      return self.genex.tasks.add(self.__meta.name, name, usage, fn);
    };

    // Only safe or meaningful to add a helper in modules initialized after the
    // templates module
    if (self.genex.templates && self.genex.templates.addHelpersForModule) {
      self.addHelpers({
        ...
        'getOption': function(dotPath, def) {
          return self.getOption(self.genex.templates.contextReq, dotPath, def);
        }
      });
    }
