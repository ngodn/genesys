// Adds minimal services to the genex object replacing
...
// the DOM is ready.
//
// Adds genex to window if not already present.

/* eslint-env browser */

(function() {

  window.genex = window.genex || {};
  var genex = window.genex;

  genex.utils = genex.utils || {};
  ...
  genex.utils.post = function(uri, data, callback) {
    if (genex.prefix) {
      uri = genex.prefix + uri;
    }
    var formData = window.FormData && (data instanceof window.FormData);
    var xmlhttp = new XMLHttpRequest();
    var csrfToken = genex.csrfCookieName ? genex.utils.getCookie(genex.csrfCookieName) : 'csrf-fallback';
  ...
  // Like `genex.utils.post` but uses a GET request, with the properties of `data`
  ...
  genex.utils.get = function(uri, data, callback) {
    if (genex.prefix) {
      uri = genex.prefix + uri;
    }
    uri += '?';
  ...
  genex.utils.getCookie = function(name) {
    var match = document.cookie.match(new RegExp('(^| )' + name + '=([^;]+)'));
    return match && match[2];
  };

  // Implementation detail of `genex.utils.post` and `genex.utils.get`
  ...
  genex.utils.removeClass = function(el, className) {
    if (el.classList) {
  ...
  genex.utils.addClass = function(el, className) {
    if (el.classList) {
  ...
  genex.utils.closest = function(el, selector) {
    if (el.closest) {
  ...
  genex.utils.assign = function(obj1, obj2 /*,  obj3... */) {
    if (Object.assign) {
  ...
  // window.genex.utils.widgetPlayers['widget-name'] = function(el, data, options) {}
  //
  // Use the widget's name, like "genesys-images", NOT the name of its module.
  //
  ...
  //
  // To register a widget player for the `genesys-images` widget, write:
  //
  // `genex.utils.widgetPlayers['genesys-images'] = function(el, data, options) { ... }`
  //
  ...
  genex.utils.runPlayers = function(el) {
    var widgets = (el || document).querySelectorAll('[data-genex-widget]');
    var i;
    if (el && el.getAttribute('data-genex-widget')) {
    ...
    function play(widget) {
      if (widget.getAttribute('data-genex-played')) {
        return;
      }
      var data = JSON.parse(widget.getAttribute('data'));
      var options = JSON.parse(widget.getAttribute('data-options'));
      widget.setAttribute('data-genex-played', '1');
      // bc with the old lean module
      var player = genex.utils.widgetPlayers[data.type] || (genex.lean && genex.lean.widgetPlayers && genex.lean.widgetPlayers[data.type]);
      if (!player) {
        return;
      }
  ...
  // You can run it again with genex.utils.runPlayers() if you AJAX-load some widgets.

  genex.utils.onReady(function() {
    // Indirection so you can override `genex.utils.runPlayers` first if you want to for some reason
    genex.utils.runPlayers();
  });
