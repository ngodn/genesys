genex.define('apostrophe-video-fields', {
  ...
  construct: function(self, options) {
    ...
    self.convert = function(data, name, $field, $el, field, callback) {
      ...
      var $player = $fieldset.find('[data-genex-video-player]');
      data[name] = {
        url: $field.val(),
    ...
    self.preview = function($fieldset, value, field) {
      ...
      $fieldset.find('[data-genex-video-error]').hide();
      return genex.oembed.queryAndPlay(
        $fieldset.find('[data-genex-video-player]'),
        {
          url: value && value.url,
          ...
          if (err) {
            $fieldset.find('[data-genex-video-error="' + err + '"]').show();
