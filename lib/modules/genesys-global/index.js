...
// by separate middleware that checks for the lock flag in `genesys-global`
...
module.exports = {
  ...
  singletonWarningIfNot: 'genesys-global',
  ...
  construct: function(self, options) {
      ...
      function body(callback) {
        ...
        function find(callback) {
          ...
          return cursor.toObject(function(err, doc) {
            ...
            // genesys-pieces-orderings-bundle if we wait
    ...
    self.initGlobal = function(callback) {
      ...
      return async.series({
        ...
        fetch: function(callback) {
          ...
          return self.genex.docs.db.find({ slug: self.slug }).toArray(function(err, result) {
            if (err) {
              return callback(err);
            }
            existing = result;
        ...
        insert: function(callback) {
          ...
          return self.genex.docs.insert(req, insert, callback);
        },
        update: function(callback) {
          ...
          return async.eachSeries(existing, function(doc, callback) {
            ...
            if (modified) {
              ...
              return self.genex.docs.update(eReq, doc, callback);
            } else {
              return setImmediate(callback);
            }
          }, callback);
    ...
    self.checkWhileBusy = function(req, _global, callback) {
      var lockName = 'genesys-global-busy';
      var localeLockName;
      ...
      function considerLock(propName, lockName, callback) {
        if (_global[propName]) {
          return self.genex.locks.lock(lockName, {
        ...
        function unlock(callback) {
          return self.genex.locks.unlock(lockName, callback);
        }
    ...
    self.addGlobalToData = function(req, res, next) {
      if (arguments.length >= 3) {
        ...
        return body(function(err) {
          if (err) {
            self.genex.utils.error('ERROR loading global doc: ', err);
    ...
    // 'genesys-workflow' (the global docs must have `workflowLocale`
    // properties).
    ...
    self.whileBusy = function(fn, options) {
      ...
      return Promise.try(function() {
        ...
        return self.genex.docs.db.update(criteria, args, {
        ...
        return Promise.try(function() {
          ...
          if (locked) {
            return self.genex.locks.unlock(lockName);
          }
        });
      });
    };
    ...
    // to set the permission requirement to admin-genesys-global.
    // This is called for you.

    self.addToAdminBar = function() {
      self.genex.adminBar.add(self.__meta.name, self.pluralLabel, 'admin-' + self.name);
    };
