...
//
// Note that if genesys-templates has a project-level override
...
module.exports = function(moduleName, searchPaths, noWatch, templates, options) {
        ...
        if (fs.existsSync(p)) {
          try {
            ...
            if (!self.firstWatchFailure) {
              ...
              self.templates.genex.utils.warn('WARNING: fs.watch does not really work on this system. That is OK but you\n' +
                'will have to restart to see any template changes take effect.');
            }
            self.templates.genex.utils.error(e);
          }
        }
      });
    }
  ...
  self.getSource = function(name) {
    ...
    if (!self.templates.genex.modules[moduleName]) {
      throw new Error("Module doesn't exist: " + moduleName);
    }

    var dirs = self.templates.getViewFolders(self.templates.genex.modules[moduleName]);
