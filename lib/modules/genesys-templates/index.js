...
// which exist courtesy of [genesys-module](https://docs.apostrophecms.org/apostrophe/modules/genesys-module)
// and invoke methods of this module more conveniently for you.
//
// You may have occasion to call `self.genex.templates.safe` when
// implementing a helper that returns a value that should not be
// escaped by Nunjucks. You also might call `self.genex.templates.filter` to
...
// You may find it easier and more maintainable to call `genex.templates.addFilter(name, fn)`.
//
...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.templateApos = {
      modules: {},
      log: function(msg) {
        self.genex.utils.log.apply(self.genex, arguments);
      },
      prefix: self.genex.prefix
    };
    ...
    // genex.modules[module-name].helperName. If the alias
    // option for the module is set to "shortname", then
    // they are also visible as genex.shortname.helperName.
    ...
    // and should only be used for genex.area, genex.singleton,
    ...
    self.modulesReady = function() {
      ...
      _.each(self.templateApos.modules, function(helpers, moduleName) {
        var alias = self.genex.modules[moduleName].options.alias;
        if (alias) {
      ...
      _.each(self.templateApos.modules, function(helpers, moduleName) {
        helpers.options = self.genex.modules[moduleName].options;
      });
    ...
    // Apostrophe Nunjucks helpers such as `genex.area` are
    // attached to the `genex` object in your template.
    ...
    self.renderBody = function(req, type, s, data, module) {
      ...
      try {
        ...
        // developers expect these things (well, at least genex and __)
        ...
        env.addGlobal('genex', self.templateApos);
        ...
        self.genex.utils.error('e.stack: ', e.stack);
        throw e;
      };
      self.renderDepth--;
      if (!self.renderDepth) {
    ...
    self.addStandardFilters = function(env) {
      ...
      env.addFilter('date', function(date, format, locale) {
        ...
        var dateLocale = locale || self.genex.templates.contextReq.locale;
        var m = moment(date);
        if (dateLocale) {
          m.locale(dateLocale);
        }
      ...
      env.addFilter('build', self.genex.urls.build);
      ...
      env.addFilter('nlbr', function(data) {
        data = self.genex.utils.globalReplace(data, "\n", "<br />\n");
        return self.genex.templates.safe(data);
      });
      ...
      env.addFilter('css', function(s) {
        return self.genex.utils.cssName(s);
      });

      env.addFilter('clonePermanent', function(o, keepScalars) {
        return self.genex.utils.clonePermanent(o, keepScalars);
      });
      ...
      env.addFilter('jsonAttribute', function(data, options) {
        if (typeof (data) === 'object') {
          return self.safe(self.genex.utils.escapeHtml(JSON.stringify(data), options));
        } else {
          // Make it a string for sure
          data += '';
          return self.safe(self.genex.utils.escapeHtml(data, options));
        }
      });
      ...
      // genex.utils.isBlessed and the same additional arguments. See
      // self.genex.utils.bless.
      ...
      env.addFilter('bless', function(options /* , 'widget', widget.type... */) {
        ...
        self.genex.utils.bless.apply(self.genex.utils, args);
        return options;
      });
    };

    // Typically you will call the `sendPage` method of
    // your own module, provided by the `genesys-module`
    ...
    self.renderPageForModule = function(req, template, data, module) {
      ...
      var globalCalls = self.genex.push.getBrowserCalls('always');
      if (scene === 'user') {
        globalCalls += self.genex.push.getBrowserCalls('user');
      }
      ...
      if (req.query.apos_refresh) {
        // Trigger the genex.ready and genex.enhance events after the
        // DOM settles and pushed javascript has had a chance to run;
        // do it just in the context of the refreshed main content div
        req.browserCall('genex.pageReadyWhenCalm($("[data-genex-refreshable]"))');
      } else if (decorate) {
        // Trigger the genex.ready and genex.enhance events after the
        // DOM settles and pushed javascript has had a chance to run
        req.browserCall('genex.pageReadyWhenCalm($("body"));');
      } else {
        // If we're ajaxing something other than data-genex-refreshable,
      ...
      if (req.user) {
        req.browserCall('genex.user = ?;', _.pick(req.user, 'title', '_id', 'username'));
      }

      req.browserCall('genex.scene = ?', scene);
      ...
      var args = {
        outerLayout: decorate ? 'genesys-templates:outerLayout.html' : 'genesys-templates:refreshLayout.html',
        permissions: (req.user && req.user._permissions) || {},
        when: scene,
        js: {
      ...
      function error(e, type) {
        ...
        self.genex.utils.error(':: ' + now + ': ' + type + ' error at ' + req.url);
        self.genex.utils.error('Current user: ' + (req.user ? req.user.username : 'none'));
    ...
    self.addBodyDataAttribute = function(req, name, value = '') {
      ...
      _.each(values, (value, key) => {
        ...
        ('data-' + (!_.isUndefined(value) && value.toString().length > 0 ? (self.genex.utils.escapeHtml(key) + ('="' + self.genex.utils.escapeHtml(value) + '"')) : self.genex.utils.escapeHtml(key)));
      });
    };
    ...
    // `genex.templates.prepended('location')` call, with the same location, to
    ...
    self.prepend = function(location, helperFn) {
      if ((typeof helperFn) === 'string') {
        throw new Error('Do not pass a string to genex.templates.prepend. Pass a function which returns a string and optionally takes a req argument.');
      }
    ...
    self.append = function(location, helperFn) {
      if ((typeof helperFn) === 'string') {
        throw new Error('Do not pass a string to genex.templates.append. Pass a function which returns a string and optionally takes a req argument.');
      }
      return self.insert('append', location, helperFn);
    };

    // Implementation detail of `genex.templates.prepend` and `genex.templates.append`.
    ...
    // Implementation detail of `genex.templates.prepended` and `genex.templates.appended`.

    self.inserted = function(end, location) {
      var key = end + '-' + location;
      return self.genex.templates.safe(_.map(self.insertions[key] || [], function(helperFn) {
        return helperFn(self.genex.templates.contextReq);
      }).join('\n'));
    };
    ...
    // (HTML tags are allowed). Available as the `genex.templates.prepended`
    ...
    // location and returns the resulting markup as a Nunjucks "safe" string
    // (HTML tags are allowed). Available as the `genex.templates.appended`
    // helper, and invoked as such by `outerLayoutBase.html`.
