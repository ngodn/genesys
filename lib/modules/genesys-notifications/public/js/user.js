genex.define('apostrophe-notifications', {

  extend: 'apostrophe-context',

  afterConstruct: function(self) {
    genex.notify = self.trigger;

    genex.on('modalStackPop', self.reparentContainer);
    genex.on('modalStackPush', self.reparentContainer);
  ...
  construct: function(self, options) {
    ...
    //
    // This method is aliased as `genex.notify` for convenience.
    //
    ...
    self.enable = function() {
      // wait for genex.user to be testable
      setImmediate(go);
      function go() {
        if (!(genex.user && genex.user._id)) {
        ...
        function poll() {
          self.api('poll-notifications', {
            ...
            _.each(data.dismissed, function(id) {
              var $notification = $('[data-genex-notification-id="' + id + '"]');
    ...
    // You want `genex.notify`, not this method, unless you are
    ...
    self.display = function(notification) {
      ...
      if (notification.type) {
        $notification.addClass('genex-notification--' + notification.type);
      }

      if (notification.pulse) {
        $notification.addClass('genex-notification--pulse');
      }
      ...
      $notification.attr('data-genex-notification-id', notification._id);

      if (notification.dismiss) {
        $notification.attr('data-genex-notification-dismiss', notification.dismiss);
    ...
    self.createContainer = function() {
      self.$container = $('<div class="genex-notification-container" data-genex-notification-container></div>');
      self.$container.hide();
    ...
    self.addToContainer = function($notification) {
      ...
      setTimeout(function() {
        $notification.removeClass('genex-notification--hidden');
      }, 100);
    };

    self.dismiss = function($notification, fromServer) {

      var _id = $notification.attr('data-genex-notification-id');
      ...
      $notification.addClass('genex-notification--hidden');

      setTimeout(function() {
        $notification.remove();
        if (!self.$container.find('[data-genex-notification]').length) {
          self.$container.hide();
        }
      }, 300);
    };
