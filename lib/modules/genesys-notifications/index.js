...
// either on the browser or the server side, via `genex.notice`.
//
// ## Options
//
...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.apiRoute('post', 'trigger', function(req, res, next) {
      ...
      self.genex.utils.readOnlySession(req);
      var type = self.genex.launder.select(req.body.type, [ 'error', 'warn', 'success', 'info' ], 'info');
      var message = self.genex.launder.string(req.body.message);
      var strings = self.genex.launder.strings(req.body.strings);
      var dismiss = self.genex.launder.integer(req.body.dismiss);
      var pulse = self.genex.launder.boolean(req.body.pulse);
    ...
    // `/modules/genesys-global/poll-notifications`. It is implemented
    ...
    self.expressMiddleware = function(req, res, next) {
      ...
      var start = Date.now();
      var displayingIds = self.genex.launder.ids(req.body.displayingIds);
      return attempt();
    ...
    self.ensureCollection = function(callback) {
      self.db = self.genex.db.collection('aposNotifications');
      return callback(null);
    };
  ...
  afterConstruct: function(self, callback) {
    ...
    self.genex.notify = self.trigger;
