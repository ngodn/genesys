...
// see the `browserCall` method of this module (`genex.push.browserCall`) as documented
// below.
//
...
// as `genex.push.browserCall`, except that there is no `when` argument.
//
// Example:
//
// ```
// req.browserCall('genex.someModule.method(?, ?)', arg1, arg2, ...)
// ```
//
...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.genex.app.request.browserCall = function(pattern) {
      var req = this;
    ...
    self.genex.app.request.getBrowserCalls = function(pattern) {
      var req = this;
    ...
    //
    // Example:
    // `genex.push.browserCall('user', 'myObject.addType(?)', typeObject)`

    self.browserCall = function(when, pattern /* , arg1, arg2... */) {
      if (arguments.length < 2) {
        throw new Error('genex.push.browserCall was invoked with only one argument.');
      }
      var args = Array.prototype.slice.call(arguments);
    ...
    // A convenience wrapper for invoking genex.mirror
    ...
    // object to genex.mirror on the browser side, although
    ...
    // This is useful to define related types, like `genesys-pieces-manager-modal`.
    ...
    self.browserMirrorCall = function(when, object, options) {
      ...
      _.each(object.__meta.chain, function(entry) {
        var finalName;
        var baseName;
        if (self.genex.synth.isMy(entry.name)) {
          baseName = self.genex.synth.myToOriginal(entry.name);
          var baseFinalName = substitute[baseName] || baseName;
          finalName = self.genex.synth.originalToMy(baseFinalName);
        } else {
          baseName = entry.name;
      ...
      return self.browserCall(when, 'genex.mirror(?)', meta);
        ...
        while (true) {
          ...
          if (qat !== -1) {
            ...
            if (pattern.charAt(qat) === '?') {
              ...
              try {
                code += self.genex.templates.jsonForHtml(call.arguments[n++]);
              } catch (e) {
                self.genex.utils.error(call.arguments);
                throw e;
              }
            } else {
    ...
    self.addHelpers({
      ...
      // (for instance: genesys-places map calls).
    ...
    var superReqGetBrowserCalls = self.genex.app.request.getBrowserCalls;
    self.genex.app.request.getBrowserCalls = function() {
