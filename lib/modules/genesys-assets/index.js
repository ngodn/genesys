...
// the [genesys-module](../genesys-module/README.md) base class.
//
...
// ### `lean`
//
// If this option is set to `true`, Apostrophe will *not* push any assets to an anonymous, logged-out site visitor, except for those pushed with `{ when: 'lean' }`. By default this includes only a tiny subset of the `genex.utils` library with necessary services to make widget players possible, with no library dependencies.
//
// Note that this means assets pushed with `{ when: 'always' }` will *not* be received, except by logged-in users.
//
// There are also no widget players, except for modules that allow you to opt in to a lean widget player by passing the `player: true` option when configuring those modules. This is currently supported by `genesys-video-widgets`.
//
// ### `static`
//
...
// See [Deploying Apostrophe in the Cloud with Heroku](https://docs.apostrophecms.org/apostrophe/tutorials/howtos/deploying-genesys-in-the-cloud-with-heroku)
...
// have to do.
// See [Deploying Apostrophe in the Cloud with Heroku](https://docs.apostrophecms.org/apostrophe/tutorials/howtos/deploying-genesys-in-the-cloud-with-heroku) for more information.
//
...
module.exports = {
  ...
  singletonWarningIfNot: 'genesys-assets',

  afterConstruct: function(self) {
    self.genex.tasks.add('apostrophe', 'generation',
    ...
    self.pushDefaults();
    self.isGenerationTask = (self.genex.argv._[0] === 'apostrophe:generation');
    self.pushCreateSingleton();
  },

  beforeConstruct: function(self, options) {
    if (options.minify === undefined) {
      options.minify = options.genex.launder.boolean(process.env.APOS_MINIFY);
    }
  ...
  construct: function(self, options) {
    ...
    if ((process.env.APOS_BUNDLE === '1') || (options.genex.argv['create-bundle'] === true)) {
    ...
    self.setDefaultScripts = function() {
      ...
      self.scripts = [
        ...
        // Core functionality of the browser-side `genex` object
    ...
    self.determineGeneration = function() {
      ...
      if (self.isGenerationTask) {
        ...
        generation = self.genex.utils.generateId();

        var bundle = self.genex.argv['create-bundle'] || self.simpleBundle;
        if (bundle) {
          if (bundle === true) {
            self.toBundleName = 'assets-' + generation;
            self.toBundle = self.genex.rootDir + '/data/temp/' + self.toBundleName;
          } else {
            self.toBundleName = bundle;
            self.toBundle = self.genex.rootDir + '/' + self.toBundleName;
          }
        ...
        if (self.genex.argv['create-bundle'] !== true) {
    ...
    self.enableBundles = function() {
      ...
      self.generations = self.genex.db.collection('apostropheGenerations');
      // Used to identify the current generation, based on a fixed _id, separate in purpose
      // from self.generations
      self.generationCollection = self.genex.db.collection('aposGeneration');
    ...
    self.uploadfsBundleCleanup = function() {
      ...
      function cleanup() {
          ...
          if (locked) {
            return self.genex.locks.unlock(self.__meta.name, function(_err) {
              if (err || _err) {
                ...
                self.genex.utils.error(err || _err);
              }
            });
          } else {
            if (err) {
        ...
        function insert(callback) {
          ...
          var current = {
            _id: self.generation,
            copies: self.enumerateCopies(self.extractedTarFiles ? { files: self.extractedTarFiles, prefix: './public' } : (self.genex.rootDir + '/' + self.toBundleName + '/public'), '/assets/' + self.generation),
            when: new Date()
          };
    ...
    self.extractBundle = function(name) {
      self.recursiveCopy(self.genex.rootDir + '/' + name, self.genex.rootDir);
    };
    ...
    // pushAsset method of genesys-module, the base class
    // of all modules.
    //
    ...
    self.afterInit = function(callback) {
      if (self.genex.isTask() && !self.isGenerationTask) {
      ...
      return self.genex.locks.withLock(self.__meta.name + ':devAssetBuild', function(callback) {
        return async.series([
          self.symlinkModules,
      ...
      if (fs.existsSync(to)) {
        ...
        if (stat.isSymbolicLink()) {
          try {
            ...
            self.genex.utils.error('WARNING: old style symbolic link exists at ' + to + ' and I do not have the privileges to remove it. You probably ran this site as Administrator before. Remove that symbolic link and the others in that folder as Administrator and try again as this non-Administrator user.');
            process.exit(1);
          }
        } else {
          rimraf.sync(to);
    ...
    self.buildLessMasters = function(callback) {
      ...
      // are retroactive to the very first use of the mixin. So genesys-ui-2
              ...
              self.lessCssWatches.push(fs.watch(absolutePath, {
                ...
                if (fs.existsSync(masterFile)) {
                  ...
                  try {
                    ...
                    self.genex.utils.warn('Unable to touch master CSS file, you might have to restart yourself to see new changes:', e);
                  }
                }
              }));
            }
    ...
    self.minify = function(callback) {
      ...
      return self.forAllAssetScenes(function(scene, callback) {
        return async.eachSeries([ 'stylesheets', 'scripts' ], function(type, callback) {
          self.purgeExcept('/genex-minified/' + scene + '-*.' + self.typeMap[type], '-' + self.generation);
          var file = self.getAssetRoot() + '/public/genex-minified/' + scene + '-' + self.generation + '.' + self.typeMap[type];
          ...
          if (!needed) {
            needed = true;
            self.genex.utils.log('MINIFYING, this may take a minute...');
          }
        ...
        if (needed) {
          self.genex.utils.log('Minification complete.');
        }
        return callback(null);
      });
    };
    ...
    self.outputAndBless = function(callback) {
      _.each(self.minified, function(byType, scene) {
        _.each(byType, function(content, type) {
          ...
          var dir = self.getAssetRoot() + '/public/genex-minified';
          if (!fs.existsSync(dir)) {
            fs.mkdirSync(dir);
          }
    ...
    self.minifySceneAssetType = function(scene, type, minifier, callback) {
      ...
      var cache = self.genex.caches.get('minify');
      return async.series({
        checkCache: function (callback) {
          ...
          return async.map(assets, function(asset, callback) {
            ...
            return self.genex.utils.md5File(asset.file, callback);
          }, function(err, md5s) {
            if (err) {
              return callback(err);
            }
    ...
    self.minifyScript = function(script, callback) {
      ...
      if (!exists) {
        self.genex.utils.warn("Warning: " + script.file + " does not exist");
        return callback(null);
      }
      if (script.preshrunk) {
      ...
      try {
        ...
        if (e.message && e.filename && e.line && e.col) {
          ...
          self.genex.utils.error(message);
          throw e;
        }
      }
    ...
    self.compileStylesheet = function(stylesheet, callback) {
      ...
      if (!exists) {
        self.genex.utils.warn('WARNING: stylesheet ' + stylesheet.file + ' does not exist');
        return callback(null, '');
      }
      ...
      return less.render(fs.readFileSync(src, 'utf8'), lessOptions, function(err, css) {
        if (err) {
          self.genex.utils.error('LESS CSS ERROR:');
          self.genex.utils.error(err);
        }
        css = css.css;
        if (self.genex.argv['sync-to-uploadfs']) {
          css = self.prefixCssUrlsWith(css, self.uploadfs().getUrl() + '/assets/' + self.generation);
        } else if (self.genex.prefix) {
    ...
    self.enableCsrf = function() {
      ...
      self.genex.push.browserCall('always', 'genex.csrf()');
    };
    ...
    self.enableLessMiddleware = function() {
      var lessMiddlewareOptions = {
        ...
        render: {
          sourceMap: {
            ...
            sourceMapBasepath: self.genex.rootDir + '/public',
      ...
      self.lessMiddleware = lessMiddleware(self.genex.rootDir + '/public', lessMiddlewareOptions,
        {
          // parser options
        },
        {
    ...
    self.prefixCssUrls = function(css) {
      return self.prefixCssUrlsWith(css, self.genex.prefix);
    };
    ...
    self.servePublicAssets = function() {
      ...
      middleware.push(self.genex.express.static(
        self.genex.rootDir + '/public',
        self.options.static || {}
      ));
      self.expressMiddleware = {
        // Run really early, before all of the stuff genesys-express normally
        // puts in, for performance reasons. Preempts expensive
        // queries related to `genesys-global` on every static file
        when: 'beforeRequired',
    ...
    self.getCoreAposProperties = function(when) {
      ...
      var properties = _.assign(
        _.pick(self.genex, 'prefix', 'csrfCookieName'),
        (when === 'user')
          ? {
          // A unique identifier for the lifetime of this
          // HTML page in the browser
            htmlPageId: self.genex.utils.generateId()
          } : {}
      );
    ...
    self.generationTask = function(callback) {
      ...
      function copyFilesToUploadfs(callback) {
        if (!(self.genex.argv['sync-to-uploadfs'] || (self.isGenerationTask && self.simpleBundle))) {
          return callback(null);
        }
    ...
    self.getUploadfsBundleTempName = function() {
      const temp = self.genex.rootDir + '/data/temp';
      if (!self.__tempMkdir) {
        self.mkdirp(temp);
    ...
    self.stylesheetsHelper = function(when) {
      if (self.options.minify) {
        return self.genex.templates.safe('<link href="' + self.assetUrl('/genex-minified/' + when + '-' + self.generation + '.css') + '" rel="stylesheet" />');
      } else {
        if (self.lessMasters && self.lessMasters[when]) {
          return self.genex.templates.safe('<link href="' + self.assetUrl(self.lessMasters[when].web) + '" rel="stylesheet" />');
        }
        return self.genex.templates.safe(_.map(self.filterAssets(self.pushed['stylesheets'], when), function(stylesheet) {
    ...
    self.scriptsHelper = function(when) {
      ...
      if (self.options.minify) {
        ...
        result += '<script src="' + self.assetUrl('/genex-minified/' + when + '-' + self.generation + '.js') + '"></script>\n';
        return self.genex.templates.safe(result);
      } else {
    ...
    self.uploadfs = function() {
      return self.genex.attachments.uploadfs;
    };

    self.addHelpers({
      // genex.assets.stylesheets renders markup to load CSS that
      ...
      // genex.assets.scripts renders markup to load JS that
      // is needed on any page that will use Apostrophe.
      //
      // `when` can be set to either `user` or `anon` and signifies
      // whether a user is logged in or not; when users are
      ...
      // See `outerLayout.html` in the templates module.
      //
      // genex.assets.scripts also creates the genex object,
    ...
    self.pushCreateSingleton = function() {
      ...
      self.genex.push.browserCall('always',
        'genex.create("genesys-assets", ?)',
        {
