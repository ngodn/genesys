...
// `genesys-search` page type for the `/search` page, which
// you should include in your "parked pages" if you wish
// to have one (see [genesys-pages](https://docs.apostrophecms.org/apostrophe/modules/genesys-pages)).
//
...
// template of `genesys-pages` contains this element:
//
...
// take an array argument and push new type names on it. `genesys-pieces` modules
...
module.exports = {
  ...
  afterConstruct: function(self) {
    ...
    self.enableFilters();
    self.pushAssets();
    self.genex.tasks.add(self.__meta.name, 'index', self.indexTask);
  },
  ...
  construct: function(self, options) {
    ...
    self.determineTypes = function(callback) {
      self.types = self.options.types || _.pluck(self.genex.pages.typeChoices, 'name');
    ...
    self.renderRoute('get', 'suggest', function(req, res, next) {
      return self.suggest(req, self.genex.launder.string(req.query.q)).then(function(docs) {
        return next(null, {
          template: 'suggest',
          data: {
            docs: docs
    ...
    self.suggest = function(req, q) {
      return self.genex.docs.find(req, {}, { _url: 1, title: 1 })
    ...
    self.indexPage = function(req, callback) {
      ...
      var cursor = self.genex.docs.find(req, {})
      ...
      return async.series([ totalDocs, findDocs ], function(err) {
        ...
        if (self.genex.utils.isAjaxRequest(req)) {
      ...
      function findDocs(callback) {
        ...
        function getDocs(callback) {
          ...
          return async.eachSeries(_.keys(byType), getDocsOfType, function(err) {
            ...
            docs = self.genex.utils.orderById(_.pluck(idsAndTypes, '_id'), docs);
            return callback(null);
          });
        }
    ...
    self.indexDoc = function(req, doc) {
      ...
      var titleSortified = self.genex.utils.sortify(doc.title);
    ...
    // Implements the `genesys-search:index` task, which re-indexes all pages.
    ...
    self.indexTask = function(genex, argv, callback) {
      var req = self.genex.tasks.getReq();
      return self.genex.migrations.eachDoc({}, _.partial(self.indexTaskOne, req), callback);
    };

    // Indexes just one document as part of the implementation of the
    // `genesys-search:index` task. This isn't the method you want to
    // override. See `indexDoc` and `getSearchTexts`

    self.indexTaskOne = function(req, doc, callback) {
      self.indexDoc(req, doc);
      return self.genex.docs.db.update({ _id: doc._id }, doc, callback);
    };
    ...
    self.getSearchTexts = function(doc) {
      ...
      self.genex.emit('docSearchIndex', doc, texts);

      // Areas can be schemaless so find them automatically
      self.genex.areas.walk(doc, function(area, dotPath) {
        ...
        _.each(area.items, function(item) {
          var manager = self.genex.areas.getWidgetManager(item.type);
          if (!manager) {
            self.genex.areas.warnMissingWidgetType(item.type);
            return;
          }
    ...
    // through genex.utils.sortify to eliminate unwanted characters and case differences

    self.boilTexts = function(texts) {
      var text = _.reduce(texts, function(memo, text) {
        return memo + ' ' + text.text;
      }, '');
      text = self.genex.utils.sortify(text);
      return text;
    };
