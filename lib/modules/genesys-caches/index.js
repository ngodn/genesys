...
module.exports = {
  ...
  singletonWarningIfNot: 'genesys-caches',
  ...
  construct: function(self, options) {
    ...
    // var myCache = self.genex.caches.get('weather-data');
    //
    ...
    self.getCollection = function(callback) {
      return self.genex.db.collection('aposCache', function(err, collection) {
        if (err) {
          return callback(err);
        }
    ...
    self.addClearCacheTask = function() {
      self.genex.tasks.add('genesys-caches', 'clear',
        'Usage: node app genesys-caches:clear cachename cachename2...\n\n' +
        'Clears caches by name. If you are using genex.caches in your own code you will\n' +
        'know the cache name. Standard caches include "genesys-migrations",\n' +
