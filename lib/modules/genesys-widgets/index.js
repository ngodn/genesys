...
// [genesys-rich-text-widgets](../genesys-rich-text-widgets/index.html),
// [genesys-pieces-widgets](../genesys-pieces-widgets/index.html) and
// [genesys-video-widgets](../genesys-video-widgets/index.html).
//
...
// It will be singular if it displays one thing, like `genesys-video`,
// and plural if it displays more than one thing, like `genesys-pieces`.
...
// (see `always.js` in `genesys-images-widgets` for an example).
//
...
// subclass of `genesys-custom-pages` or `genesys-pieces-pages`, as well.
//
...
// ### `defer`
//
// If you set `defer: true` for a widget module, like genesys-images-widgets, the join to
...
// you can also set `deferWidgetLoading: true` for the `genesys-global` module. To avoid chicken
...
// [genesys-video-widgets](../genesys-video-widgets/index.html) module:
//
// ```javascript
...
//       return genex.oembed.queryAndPlay($widget.find('[data-genex-video-player]'), data.video);
//     };
//   }
// });
// ```
...
module.exports = {
  ...
  afterConstruct: function(self) {
    ...
    if (!self.options.label) {
      throw new Error('You must specify the label option when subclassing genesys-widgets');
    }
    ...
    self.genex.areas.setWidgetManager(self.name, self);

    self.genex.push.browserMirrorCall('user', self);
    self.genex.push.browserMirrorCall('user', self, { tool: 'editor' });

    self.genex.tasks.add(self.__meta.name, 'list',
  ...
  construct: function(self, options) {
    ...
    self.composeSchema = function() {
      self.schema = self.genex.schemas.compose(options, self);
    ...
    // `genesys-areas` module as it iterates over widgets in
    ...
    self.load = function(req, widgets, callback) {
      ...
      function join(callback) {
        return self.genex.schemas.join(req, self.schema, widgets, undefined, callback);
      }
      function area(callback) {
        ...
        var cursor = self.genex.docs.find(req).joins(false);
    ...
    // `convert` method of `genesys-schemas`, creating a new `output` object
    ...
    self.sanitize = function(req, input, callback) {
      ...
      // `by` doesn't go missing for `genesys-images-widgets`
      var output = self.genex.schemas.newInstance(self.schema);
      var schema = self.allowedSchema(req);
      output._id = self.genex.launder.id(input._id) || self.genex.utils.generateId();
      return self.genex.schemas.convert(req, schema, 'form', input, output, function(err) {
        if (err) {
          return callback(err);
        }
    ...
    self.allowedSchema = function(req) {
      return _.filter(self.schema, function(field) {
        return (!field.permission) || self.genex.permissions.can(req, field.permission);
      });
    };
    ...
    self.filterForDataAttribute = function(widget) {
      var data = self.genex.utils.clonePermanent(widget, true);
    ...
    self.filterOptionsForDataAttribute = function(options) {
      return self.genex.utils.clonePermanent(options, true);
    };
    ...
    self.pushDefineSingleton = function() {
      self.genex.push.browserMirrorCall('always', self, { stop: 'genesys-widgets' });
    };
    ...
    self.list = function(genex, argv, callback) {

      return self.genex.migrations.eachWidget({}, iterator, callback);
      ...
      function iterator(doc, widget, dotPath, callback) {
        ...
        if (widget.type === self.name) {
          ...
          // not become an genex.utils.log call. -Tom
    ...
    self.addSearchTexts = function(widget, texts) {
      self.genex.schemas.indexFields(self.schema, widget, texts);
    };
