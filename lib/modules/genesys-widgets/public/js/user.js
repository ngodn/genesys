genex.define('genesys-widgets', {
  ...
  construct: function(self, options) {
    ...
    self.edit = function(data, options, save) {
      ...
      if (!data._id) {
        _.assign(data, genex.schemas.newInstance(self.schema));
      }
      ...
      function checkOrGenerateId (data) {
        data = data || {};
        if (!data._id) {
          data._id = genex.utils.generateId();
        }
        return data;
      }
    ...
    self.getData = function($widget) {
      ...
      _.each(self.schema, function(field) {
        var type = genex.schemas.fieldTypes[field.type];
        if (
          ...
          genex.schemas.fieldTypes[field.type].contextualConvert(data, field.name, $widget, field);
        }
      });
      return data;
    };
    ...
    // like genesys-rich-text, as opposed to widgets edited
    // via a modal.
