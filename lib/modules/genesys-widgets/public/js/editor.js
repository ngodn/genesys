genex.define('apostrophe-widgets-editor', {
  ...
  construct: function(self, options) {
    ...
    if (!self.data._id) {
      self.data._id = genex.utils.generateId();
    }
    self.saveContent = function(callback) {
      return async.series({
        convert: function(callback) {
          return genex.schemas.convert(self.$el, self.schema, self.data, function(err) {
            if (err) {
              return callback(err);
            }
    ...
    self.beforeShow = function(callback) {
      ...
      return async.series({
        populate: function(callback) {
          return genex.schemas.populate(self.$el, self.schema, self.data, callback);
        }
      }, callback);
    };
      ...
      return async.series({
        errorPath: function(callback) {
          if (self.errorPath && self.errorPath.length) {
            return genex.schemas.returnToError(self.$el, self.schema, self.errorPath, self.error, callback);
          } else {
            return callback(null);
          }
        },
