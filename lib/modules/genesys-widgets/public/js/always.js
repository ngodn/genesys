// `genesys-widgets` is a parent class for the browser-side managers of
// widget types. Each manager object is responsible for *all* widgets of that type.
//
// Extends `genesys-context` in order to gain access to conveniences like
...
genex.define('genesys-widgets', {
  ...
  afterConstruct: function(self) {
    ...
    genex.areas.setWidgetManager(self.name, self);
  ...
  construct: function(self, options) {
    ...
    // JSON data, but some widgets, like genesys-rich-text,
    ...
    self.canEdit = function($widget) {
      ...
      var result = data._edit && (!$widget.closest('[data-genex-area-disabled-editing]').length);
      return result;
    };
  }
});
