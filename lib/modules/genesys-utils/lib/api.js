...
module.exports = function(self, options) {
  ...
  self.genex.pid = self.generateId();
  ...
  // "search" option to genex.get and everything layered on top of it).
  ...
  //
  // `genex.utils.bless(req, options, 'widget', widget.type)`
  //
  // ON A LATER AJAX REQUEST TO THE render-widget ROUTE
  //
  // `if (genex.utils.isBlessed(req, options, 'widget', widget.type)) { /* It's safe! */ }`
  //
  ...
  // See genex.utils.bless. Checks whether the given set of arguments
  ...
  self.enableLogger = function() {
    self.logger = self.options.logger ? self.options.logger(self.genex) : require('./logger.js')(self.genex);
  };
  ...
  // `console.warn` documentation.
  //
  // The intention is that `genex.utils.warn` should be
  ...
  // Identical to `genex.utils.warn`, except that the warning is
  ...
  // Identical to `genex.utils.warnDev`, except that the warning is
  ...
  self.warnDevOnce = function(name, msg) {
    var always = self.genex.argv['all-' + name];
  ...
  //
  // Profiler modules such as `genesys-profiler` override this
  ...
  // However, the `genesys-profiler` module extends them to invoke it,
  ...
  // with high-resolution accuracy. Used by genex.utils.profile.
