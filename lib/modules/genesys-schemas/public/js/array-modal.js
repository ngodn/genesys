genex.define('apostrophe-array-editor-modal', {
  ...
  construct: function(self, options) {
    ...
    self.load = function() {
      self.$arrayItems = self.$el.find('[data-genex-array-items]');
      self.$arrayItem = self.$el.find('[data-genex-array-item]');
    ...
    self.addSaveHandler = function() {
      self.$save = self.$el.parents('[data-modal]').find('[data-genex-save-array-items]');
    ...
    self.addToItems = function() {
      var item = genex.schemas.newInstance(self.field.schema);
      _.assign(item, {
    ...
    self.populateItem = function() {
      var $form = self.$arrayItem.find('[data-genex-form]');
      genex.schemas.populate($form, self.field.schema, self.arrayItems[self.active], function(err) {
        if (err) {
          genex.utils.error(err);
        }
        self.refresh();
        if (self.errorPath && (self.errorPath.length > 1)) {
          genex.schemas.returnToError($form, self.field.schema, self.errorPath.slice(1), self.error, function(err) {
            if (err) {
              genex.utils.error(err);
            }
          });
    ...
    self.saveItemState = function(callback) {
      ...
      genex.schemas.convert(self.$arrayItem.find('[data-genex-form]'), self.field.schema, self.arrayItems[self.active], function(err) {
        if (err) {
          return callback(err);
        }
        self.setItemTitles();
    ...
    self.bindClickHandlers = function() {
      self.$el.on('click', '[data-genex-array-items-trigger]', function(e) {
        e.stopPropagation();
        e.preventDefault();
        var $trigger = $(this);

        var index = $trigger.data('genex-array-items-trigger');
      ...
      if (options.field.readOnly) {
        ...
        self.$el.children().find('[data-genex-add-array-item]').remove();
        self.$el.parents('[data-modal]').find('[data-genex-save-array-items]').remove();
      } else {
        self.$el.on('click', '[data-genex-add-array-item]', function(e) {
          e.stopPropagation();
          e.preventDefault();
          self.saveItemState(function(err) {
            if (err) {
        ...
        self.$el.on('click', '[data-genex-move-array-item]', function(e) {
          e.stopPropagation();
          e.preventDefault();
          var $el = $(this);
          ...
          self.saveItemState(function(err) {
            ...
            var direction = $el.data('genex-move-array-item');
        ...
        self.$el.on('click', '[data-genex-delete-array-item]', function(e) {
          e.stopPropagation();
          e.preventDefault();
          ...
          // before this one.
          if (self.active === parseInt($self.closest('[data-genex-array-items-trigger]').attr('data-genex-array-items-trigger'))) {
    ...
    self.limit = function() {
      ...
      if (self.arrayItems.length >= self.field.limit) {
        self.$el.find('[data-genex-add-array-item]').hide();
        self.$el.find('[data-genex-array-limit-reached]').show();
      } else {
        self.$el.find('[data-genex-add-array-item]').show();
        self.$el.find('[data-genex-array-limit-reached]').hide();
      }
    };
    ...
    self.required = function() {
      ...
      if (self.arrayItems.length) {
        ...
        self.$save.removeClass('genex-button--disabled');
      } else {
        self.$save.attr('disabled', '');
        self.$save.addClass('genex-button--disabled');
      }
    };
