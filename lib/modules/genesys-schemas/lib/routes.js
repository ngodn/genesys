...
module.exports = function(self, options) {
  ...
  self.routes.arrayItems = function(req, res) {
    ...
    if (!self.genex.utils.isBlessed(req, req.body.field, 'array')) {
      self.genex.utils.warn('unblessed array: ' + JSON.stringify(req.body.field, null, '  '));
      return self.apiResponder(req, 'invalid');
    }
    var receptacle = {};
    var source = {};
    var name = req.body.field.name;
    var active = self.genex.launder.integer(req.body.active);
    ...
    return async.series([ convert, join ], function(err) {
      ...
      if (Array.isArray(req.body.arrayItems)) {
        _.each(receptacle[name], function(item, i) {
          item._ordinal = self.genex.launder.integer(req.body.arrayItems[i]._ordinal);
        });
      }
  ...
  self.routes.choices = function(req, res, next) {
    ...
    if (!self.genex.utils.isBlessed(req, field, 'field')) {
      return next(new Error('Unblessed choices request for field: ', field));
    }
    return Promise.try(function() {
      return self.genex.modules[field.moduleName][field.choices](req);
