      ...
      after: function(results, callback) {
          ...
          if (areasInfo.length) {
            ...
            if (req.areasLoadedFor[doc._id] >= 5) {
              if (!req.suppressAreaLoaderRecursionWarnings) {
                self.genex.utils.warn('WARNING: reached maximum area loader recursion level. Doc _id is ' + doc._id + '. Are you using projections for all joins?');
              }
        ...
        if (req.deferWidgetLoading && (!req.loadingDeferredWidgets)) {
          ...
          _.each(types, function(type) {
            var manager = self.genex.areas.getWidgetManager(type);
            if (!manager) {
              self.genex.areas.warnMissingWidgetType(type);
    ...
    self.addFilter('originalWidgets', {
      ...
      after: function(results) {
        _.each(results, function(result) {
          ...
          self.genex.areas.walk(result, function(area, dotPath) {
            _.each(area.items || [], function(item) {
