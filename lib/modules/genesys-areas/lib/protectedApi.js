      ...
      _.each(area.items || [], function(item, i) {
        if (doc._originalWidgets) {
          try {
            ...
            self.genex.utils.error('May be bad widget data in doc ' + doc._id + ' with dotPath ' + dotPath + '.' + i + ', disregarding this widget');
          }
        }
      });
    });
  ...
  self.restoreDisallowedFields = function(req, item, oldItem) {
    ...
    if (oldItem) {
      _.each(disallowed, function(field) {
        var fieldType = self.genex.schemas.fieldTypes[field.type];
        if (!fieldType) {
