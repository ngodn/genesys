...
module.exports = function(self, options) {
  ...
  // this method is called for you when you extend the `genesys-widgets`
  ...
  self.warnMissingWidgetType = function(name) {
    ...
    if (!self.missingWidgetTypes[name]) {
      self.genex.utils.error('WARNING: widget type ' + name + ' exists in content but is not configured');
  ...
  // `genex.area` and `genex.singleton` template helpers.
  ...
  self.renderArea = function(area, options) {
    ...
    if (self.genex.templates.contextReq.disableEditing) {
  ...
  self.saveArea = function(req, docId, dotPath, items, callback) {
    ...
    return async.series({
      find: function(callback) {
        return self.genex.docs.find(req, { _id: docId })
          .permission('edit-doc')
          .published(null)
      ...
      update: function(callback) {
        ...
        if (_.isEqual(self.genex.utils.clonePermanent(items), self.genex.utils.clonePermanent(existingItems))) {
        ...
        return self.genex.docs.update(req, doc, callback);
      }
    }, function(err) {
      return callback(err);
    });
  ...
  self.lockSanitizeAndSaveArea = function(req, areaInfo, callback) {
    ...
    var docId = self.genex.launder.id(areaInfo.docId);
    var dotPath = self.genex.launder.string(areaInfo.dotPath);
    ...
    return async.series([
      ...
      if (hardLocked) {
        return self.genex.locks.unlock('save-area-' + docId, function(_err) {
    ...
    function hardLock(callback) {
      return self.genex.locks.lock('save-area-' + docId, { waitForSelf: true }, function(err) {
        if (err) {
          return callback(err);
        }
        hardLocked = true;
    ...
    function advisoryLock(callback) {
      ...
      return self.genex.docs.lock(req, docId, req.htmlPageId, callback);
    }
  ...
  self.walk = function(doc, iterator) {
    return self.genex.docs.walk(doc, function(o, k, v, dotPath) {
      if (v && (v.type === 'area')) {
  ...
  // `genesys-pieces` in a page template.
  ...
  self.getSchemaOptions = function(doc, name) {
    ...
    var manager = self.genex.docs.getManager(doc.type);
    if (!manager) {
  ...
  //
  // Also available as a helper via `genex.areas.richText(area, options)` in templates.
  //
  ...
  // if you wish.
  //
  // Also available as a helper via `genex.areas.plaintext(area, options)` in templates.
  //
  // Content will be retrieved from any widget type that supplies a
  // `getRichText` method.
  ...
  self.plaintext = function(within, options) {
    ...
    var plaintext = self.genex.utils.htmlToPlaintext(richText).trim();
    ...
    return self.genex.utils.truncatePlaintext(plaintext, options.limit, ellipsis);
  };

  // Very handy for imports of all kinds: convert plaintext to an area with
  // one `genesys-rich-text` widget if it is not blank, otherwise an empty area. null and
  ...
  self.fromPlaintext = function(plaintext, options) {
    ...
    if (plaintext && ((typeof plaintext) === 'string') && plaintext.length) {
      ...
      if (options && options.el) {
        content = `<${options.el}>${self.genex.utils.escapeHtml(plaintext, true)}</${options.el}>`;
      } else {
        content = self.genex.utils.escapeHtml(plaintext, true);
      }
      area.items.push({
  ...
  // Convert HTML to an area with one 'genesys-rich-text' widget, otherwise
  ...
  // genex.areas.richText() method which returns just the rich text of the area
  ...
  // also at the end of the `genesys-global` middleware.
  ...
  self.widgetControlGroups = function(req, widget, options) {
    ...
    var controlGroups = ([
      ...
      {
        classes: 'genex-button-group--data',
        controls: [
          {
            label: '$editLabel',
            action: 'edit-item'
  ...
  self.addSchemaWidgetControls = function(req, widget, controlGroups) {
    ...
    return controlGroups.concat([{
      classes: 'genex-widget-schema-controls',
      controls: schema
    }]);
  };
  ...
  // merge new methods with all genesys-cursors
  self.genex.define('genesys-cursor', require('./cursor.js'));
};
