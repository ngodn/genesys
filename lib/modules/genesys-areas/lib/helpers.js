...
module.exports = function(self, options) {
  self.addHelpers({
    ...
    // `genex.singleton` renders a single widget of a fixed type, standing alone
    ...
    // mind later and call `genex.area` with the same `name`.
    //
    ...
    // very convenient when working with `genesys-pieces`.
    //
    ...
    singleton: function(doc, name, type, _options) {
      ...
      if (arguments.length === 1) {
        ...
        if (!name.match(/^\w+$/)) {
          self.genex.utils.warn(name + ' in ' + doc.slug + ': area names should be made up only of letters, underscores and digits. Otherwise they may not save properly.');
        }
        options = { type: type };
        if (_.has(doc, name)) {
          area = doc[name];
      ...
      if (area._edit) {
        _.each(options.widgets || {}, function(options, type) {
          self.genex.utils.bless(self.genex.templates.contextReq, options, 'widget', type);
        });
      }
    ...
    // `genex.area` renders an area: a column of widgets of one or more types.
    //
    ...
    // in `genex.singleton`. See the documentation above for `addLabel`,
    ...
    area: function(doc, name, _options) {
      ...
      options.hint = options.hint || self.genex.templates.contextReq.__('Use the Add Content button to get started.');
    ...
    // genex.areas.widget renders one widget. Invoked by both `genex.area` and
    // `genex.singleton`. Not
    ...
    // Returns the rich text markup of all `genesys-rich-text` widgets
    ...
    // providers of rich text, such as subclasses of `genesys-rich-text`,
    ...
    richText: function(within, options) {
      ...
      return self.genex.templates.safe(self.richText(within, options));
    },
    ...
    widgetControlGroups: function(widget, options) {
      return self.widgetControlGroups(self.genex.templates.contextReq, widget, options);
    },
