...
module.exports = function(self, options) {
  var launder = self.genex.launder;

  self.apiRoute('post', 'save-area', function(req, res, next) {
    return self.lockSanitizeAndSaveArea(req, req.body, function(err, info) {
      if (err === 'locked-by-me') {
        return next('locked', null, { message: self.genex.i18n.__('You are now editing this document in another tab or window.') });
      } else if (err === 'locked') {
        return next('locked', null, { message: self.genex.i18n.__('Another user has taken control of this document.') });
      } else {
        return next(err);
      }
    });
  ...
  self.apiRoute('post', 'save-areas-and-unlock', function(req, res, next) {
    ...
    function unlockAll(callback) {
      ...
      return self.genex.docs.unlockAll(req, req.htmlPageId, callback);
    }
  });
  ...
  self.renderRoute('post', 'edit-virtual-area', function(req, res, next) {
    ...
    var area = {
      type: 'area',
      _docId: 'v' + self.genex.utils.generateId(),
      _edit: true
    };
    ...
    return async.eachSeries(items, function(item, callback) {
      ...
      if (!self.genex.utils.isBlessed(req, widgetOptions, 'widget', item.type)) {
        return callback('The widget options are not blessed. Usually this means you are\n' +
          ...
          'last argument to "genex.area" in your template.\n\n' +
  ...
  self.htmlRoute('post', 'render-widget', function(req, res, next) {
    ...
    if (!self.genex.utils.isBlessed(req, options, 'widget', type)) {
