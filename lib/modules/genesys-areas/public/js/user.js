genex.define('genesys-areas', {
  ...
  construct: function(self, options) {
    ...
    // When a `[data-genex-add-content]` button is clicked, toggle the `genex-active` class
    // on the closest ancestor with `[data-genex-dropdown]`, and also the closest
    // ancestor with `.genex-area-controls`.
    self.enhanceAddContent = function() {
      var $body = $('body');
      $body.on('click', '[data-genex-add-content]', function(e) {
        var $area = $(this).closest('.genex-area');
        var $dropdown = $(this).closest('[data-genex-dropdown]');
        if ($dropdown.hasClass('genex-active')) {
          // Call this consistently in all places we dismiss the menu
          $area.data('editor').dismissContextContentMenu();
        } else {
          $(this).closest('[data-genex-dropdown]').addClass('genex-active');
          $(this).closest('.genex-area-controls').addClass('genex-active');
          $(this).closest('.genex-area').addClass('genex-content-menu-active');
          ...
          $('.genex-context-menu-container').addClass('genex-content-menu-active');
    ...
    self.enhanceControlsHover = function(sel) {
      $(sel).find('[data-genex-widget]').addBack('[data-genex-widget]').each(function() {
        var $widget = $(this);
        $widget.on('mouseenter', '[data-genex-tooltip]', genex.ui.createTooltip);
        $widget.on('mouseleave click', '[data-genex-tooltip]', genex.ui.removeTooltip);
      ...
      function mouseoverEnhance($el) {
        $el.on('mouseover', function(e) {
          var $self = $(this);
          e.stopPropagation();
          $('.genex-area-widget-controls').removeClass('genex-peek');
          $self.find('[data-genex-widget-controls]:first .genex-area-widget-controls').addClass('genex-peek');
          $self.next().children().addClass('genex-peek');
        });
      };
      ...
      function mouseleaveEnhance($el) {
        $el.on('mouseout', function(e) {
          var $self = $(this);
          $self.find('[data-genex-widget-controls]:first .genex-area-widget-controls').removeClass('genex-peek');
          $self.next().children().removeClass('genex-peek');
        });
      }
    ...
    self.fromTemplate = function(sel) {
      return $('[data-genex-area-editor-templates]').find(sel).clone();
    };
    ...
    self.enableCkeditor = function() {
      ...
      CKEDITOR.plugins.addExternal('split', genex.prefix + self.options.action + '/js/ckeditorPlugins/split/', 'plugin.js');
    ...
    self.enableOnEnhance = function() {
      genex.on('enhance', function($context) {
    ...
    self.enableAll = function(sel) {
      ...
      $(sel).find('[data-genex-area-edit]').each(function() {
        var $el = $(this);
        ...
        if (_.has(self.editors, areaId)) {
          ...
          genex.create('genesys-areas-editor', { $el: $el, action: self.options.action });
        }
      });
    ...
    self.enableShift = function() {
      ...
      genex.on('shiftDown', function() {
        $('[data-genex-move-item]').each(function() {
          var $self = $(this);
          if ($self.attr('data-genex-move-item') === 'up') {
            $self.children('i').toggleClass('icon-double-angle-up');
            $self.attr('data-genex-move-item', 'top');
          } else if ($self.attr('data-genex-move-item') === 'down') {
            $self.children('i').toggleClass('icon-double-angle-down');
            $self.attr('data-genex-move-item', 'bottom');
          }
        });
      });

      genex.on('shiftUp', function() {
        $('[data-genex-move-item]').each(function() {
          var $self = $(this);
          $self.children('i').removeClass('icon-double-angle-up');
          $self.children('i').removeClass('icon-double-angle-down');
          if ($self.attr('data-genex-move-item') === 'top') {
            $self.attr('data-genex-move-item', 'up');
          } else if ($self.attr('data-genex-move-item') === 'bottom') {
            $self.attr('data-genex-move-item', 'down');
          }
        });
      });
    };
    ...
    self.getWidgetOptions = function($widget) {
      var $area = $widget.closest('[data-genex-area]');
      var options = {};
    ...
    self.getWidgetData = function($widget) {
      ...
      if (!manager) {
        genex.utils.warn('WARNING: no manager for widget type ' + type);
        return;
      }
    ...
    self.setWidgetData = function($widget, data) {
      ...
      var manager = self.getWidgetManager(type);
      if (!manager) {
        genex.utils.log('WARNING: no manager for widget type ' + type);
        return;
      }
    ...
    // (subclasses of genesys-areas-editor) which
    // are currently visible.

    self.getEditors = function() {
      var $areas = $('[data-genex-area]:visible');
      var editors = [];
      $areas.each(function() {
        var $area = $(this);
