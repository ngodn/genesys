...
genex.define('genesys-areas-editor', {
  ...
  construct: function(self, options) {
    ...
    // property, such as `genesys-rich-text` (note no suffix).
    ...
    self.removeInitialContent = function($el, entireItem) {
      if (entireItem) {
        ...
        $el.find('[data-genex-widget="genesys-rich-text"]:has([data-initial-content])').remove();
      } else {
    ...
    self.stopEditingRichText = function() {
      ...
      genex.areas.getWidgetManager('genesys-rich-text').stopEditing(self.$activeRichText);
    ...
    // Override default genesys-context functionality because we
