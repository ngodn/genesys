...
// ```
//
// The `action` becomes a `data-genex-ACTIONGOESHERE` attribute on the
...
// check out the [genesys-custom-pages](https://docs.apostrophecms.org/apostrophe/modules/genesys-custom-pages)
...
// ```
//
// See the [genesys-pages-cursor](https://docs.apostrophecms.org/apostrophe/modules/genesys-pages/server-genesys-pages-cursor) type for additional
...
module.exports = {
  ...
  beforeConstruct: function(self, options) {
    ...
    ].concat(options.genex.docs.trashInSchema ? [
      {
        name: 'rescue',
        label: 'Rescue'
      }
  ...
  construct: function(self, options) {
    ...
    self.genex.define('genesys-cursor', require('./lib/anyCursor.js'));

    // Add methods to cursors used specifically to fetch pages only
    self.genex.define('genesys-pages-cursor', require('./lib/pagesCursor.js'));

    // When all modules are ready, invoke `registerGenericPageTypes` to register a manager
    // for any page type that doesn't already have one via `genesys-custom-pages`,
    // `genesys-pieces-pages`, etc.
