...
genex.define('apostrophe-pages-reorganize', {
  ...
  construct: function(self, options) {
    ...
    self.enhanceNode = function(node, $li) {
      ...
      if (!node.virtualTrashcan) {
        ...
        $li.find('.jqtree-element .genex-reorganize-title').before($checkbox);

        var $link;
        $link = $('<a class="genex-edit" target="_blank"></a>');
        $link.attr('data-node-id', node.id);

        $link.attr('data-edit', '1');
        $link.append('Settings');
        // add published indicator
        $li.find('.jqtree-element .genex-reorganize-controls--published').append('<span class="genex-reorganize-published genex-reorganize-published--' + node.published + '"></span>');

        $link.attr('href', '#');
        $li.find('.jqtree-element .genex-reorganize-controls--edit').append($link);
        ...
        if (node.publish) {
          ...
          $link = $('<a class="genex-delete"></a>');
          $link.attr('data-node-id', node.id);
          $link.attr('data-delete', '1');
          $link.attr('href', '#');
          $link.append('Move to Trash');
          $li.find('.jqtree-element .genex-reorganize-controls--trash').append($link);

          if (self.options.deleteFromTrash) {
            // "Destroy forever" button. CSS reveals when
            // already in the trash
            $link = $('<a class="genex-delete-from-trash"></a>');
            $link.attr('data-node-id', node.id);
    ...
    self.addControlGroupToNode = function(node, $li) {
      $li.find('.jqtree-element').append($('<span class="genex-reorganize-controls genex-reorganize-controls--published"></span>'));
      $li.find('.jqtree-element').append($('<span class="genex-reorganize-controls genex-reorganize-controls--edit"></span>'));
      $li.find('.jqtree-element').append($('<span class="genex-reorganize-controls genex-reorganize-controls--link"></span>'));
      $li.find('.jqtree-element').append($('<span class="genex-reorganize-controls genex-reorganize-controls--trash"></span>'));
    };

    self.indentLevel = function(node, $li) {
      // add page depth as data attr
      $li.attr('data-genex-reorganize-depth', node.getLevel());
      if (node.getLevel() !== 1) {
        $li.find('.genex-reorganize-title').css('padding-left', 15 * node.getLevel() + 'px');
      }
    };

    self.configureTitleContainer = function(node, $li) {
      var $titleContainer = $('<span class="genex-reorganize-title"></span>');
      $titleContainer.append($li.find('.jqtree-title'));
      if ($li.find('.jqtree-toggler').length === 0) {
        $titleContainer.addClass('genex-reorganize-title--no-toggle');
      } else {
    ...
    self.addVisitLink = function(node, $li) {
      ...
      var $link = $('<a class="genex-visit" target="_blank"></a>');
      $link.attr('data-node-id', node.id);
      ...
      $link.append('Link');
      $li.find('.jqtree-element .genex-reorganize-controls--link').append($link);
    };
    ...
    self.visit = function($node) {
      ...
      var tab = window.open(genex.prefix + node.slug, '_blank');
      tab.focus();
    };
    ...
    self.edit = function($node) {
      ...
      genex.create('apostrophe-pages-editor-update', {
    ...
    self.delete = function($node) {
      ...
      if (!trash) {
        genex.notify('No trashcan.', { type: 'error', dismiss: true });
        return false;
      }

      genex.pages.trash(node.id, function(err, parentSlug, changed) {
        if (err) {
          genex.notify('An error occurred.', { type: 'error', dismiss: true });
          return;
        }
        if (genex.docs.trashInSchema) {
    ...
    self.deleteFromTrash = function($node) {
      ...
      genex.pages.deleteFromTrash(node.id, function(err, parentSlug) {
        if (err) {
          genex.notify('An error occurred.', { type: 'error', dismiss: true });
          return;
        }
    ...
    self.move = function(e) {
      var trashing;
      e.preventDefault();
      self.$el.find('.genex-reorganize-progress').fadeIn();
      var data = {
      ...
      return async.series([
        ...
        e.move_info.do_move();
        self.updateVirtualTrashcans();
        self.$el.find('.genex-reorganize-progress').fadeOut();
      });
      ...
      function trash(callback) {
        ...
        if (trashing) {
          return genex.pages.trash(node.id, callback);
        } else {
          return genex.pages.rescue(node.id, callback);
        }
      }
    ...
    self.errorOnReload = function() {
      genex.notify('The server did not respond or you do not have the appropriate privileges.', { type: 'error' });
      self.hide();
    };

    self.errorOnMove = function() {
      genex.notify('You may only move pages you are allowed to publish. If you move a page to a new parent, you must be allowed to edit the new parent.', { type: 'error' });
    ...
    self.enableBatchOperations = function(callback) {
      ...
      self.link('genex-batch-operation', function($el, action) {
    ...
    self.reflectBatchOperation = function() {
      ...
      self.$batch.find('[data-genex-batch-operation-form]').removeClass('genex-active');
      self.$batch.find('[data-genex-batch-operation-form="' + val + '"]').addClass('genex-active');
      self.$batch.find('[data-genex-batch-operation]').addClass('genex-hidden');
      self.$batch.find('[data-genex-batch-operation="' + val + '"]').removeClass('genex-hidden');
      ...
      var $buttons = self.$batch.find('.genex-button');
      if (count) {
        $buttons.removeClass('genex-button--disabled');
      } else {
        $buttons.addClass('genex-button--disabled');
      }
    ...
    self.enableBatchOperation = function(batchOperation, callback) {
      ...
      batchOperation.handler = self['batch' + genex.utils.capitalizeFirst(genex.utils.camelName(batchOperation.name))];
      if (!batchOperation.schema) {
        return setImmediate(callback);
      }
      var data = genex.schemas.newInstance(batchOperation.schema);
      return genex.schemas.populate(
        self.$batch.find('[data-genex-batch-operation-form="' + batchOperation.name + '"]'),
    ...
    self.batchSimple = function(operationName, confirmationPrompt, options) {
      ...
      function convert(callback) {
        ...
        return genex.schemas.convert(
          self.$batch.find('[data-genex-batch-operation-form="' + operationName + '"]'),
          operation.schema,
          data,
          {},
          callback
      ...
      function save(callback) {
        ...
        return self.api(operation.route || operationName, data, function(result) {
          ...
          if (result.jobId) {
            var jobs = genex.modules['apostrophe-jobs'];
