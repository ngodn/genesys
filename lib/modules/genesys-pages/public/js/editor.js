...
genex.define('genesys-pages-editor', {
  extend: 'genesys-modal',
  source: 'editor',
  verb: 'insert',
  label: 'New Page',
  construct: function(self, options) {
    self.typeChoices = genex.pages.options.typeChoices;
    self.page = options.page || genex.pages.page;
    ...
    self.addTypeChangeHandler = function() {
      var $type = genex.schemas.findField(self.$el, 'type');
      ...
      return self.api(
        'fetch-to-' + self.verb,
        {
          ...
          if (result.status === 'notfound') {
            genex.utils.error('That page does not exist.');
            return callback('error');
          } else if (result.status !== 'ok') {
            genex.notify('An error occurred. Please try again.', { type: 'error' });
            return callback('error');
          }
    ...
    self.populate = function(data, callback) {
      return genex.schemas.populate(self.$el, self.schema, data, callback);
    };
    ...
    self.convert = function(page, callback) {
      return genex.schemas.convert(self.$el, self.schema, page, callback);
    };
      ...
      return async.series({
        ...
        save: function(callback) {
          return self.api(self.verb, { currentPageId: self.page._id, page: page }, function(_data) {
            data = _data;
            if (data.status !== 'ok') {
              genex.notify('An error occurred while creating the page: ' + data.status, { type: 'error' });
        ...
        redirect: function(callback) {
          if (self.options.redirectIfSamePage || (self.options.redirectIfSamePage === undefined)) {
            if (self.page._id === genex.pages.page._id) {
      ...
      return async.series({
        ...
        fetch: function(callback) {
          return self.api('fetch-to-' + self.verb, { _id: self.page._id, type: type }, function(result) {
            if (result.status === 'notfound') {
              genex.notify('That page does not exist.', { type: 'error' });
              return callback('error');
            } else if (result.status !== 'ok') {
              genex.notify('An error occurred. Please try again.', { type: 'error' });
              return callback('error');
        ...
        genex.emit('enhance', self.$el.find('[data-page-schema-fields]'));
        self.addTypeChangeHandler();
        self.type = type;
      });
    };
...
genex.define('genesys-pages-editor-update', {
  extend: 'genesys-pages-editor',
  verb: 'update',
  label: 'Edit Page Settings'
});

genex.define('genesys-pages-editor-copy', {
  extend: 'genesys-pages-editor',
  verb: 'copy',
  label: 'Copy Page'
});
