...
module.exports = function(self, options) {
  ...
  self.find = function(req, criteria, projection) {
    return self.genex.create(self.__meta.name + '-cursor', {
      genex: self.genex,
      req: req,
      criteria: criteria,
    ...
    function body(callback) {
      ...
      return async.series({
        ...
        determineNextRank: function(callback) {
          return self.genex.docs.db.findWithProjection({
        ...
        insert: function(callback) {
          var slugBasename = require('path').basename(self.genex.utils.slugify(page.slug, { allow: '/' }));
          var pathFrom;
          if (slugBasename.length && (slugBasename !== 'none')) {
            pathFrom = slugBasename;
          } else {
            pathFrom = self.genex.utils.slugify(page.title);
          }
          page.path = addSlashIfNeeded(parent.path) + pathFrom;
          page.level = parent.level + 1;
          return self.genex.docs.insert(req, page, options, callback);
        }
      }, function(err) {
        if (err) {
          return callback(err);
  ...
  self.lock = function(req, callback) {
    ...
    return self.genex.locks.lock('genesys-pages:tree', function(err) {
      if (!err) {
  ...
  self.unlock = function(req, callback) {
    if (!req.aposPageTreeLockDepth) {
      return callback(new Error('Looks like you called genex.pages.unlock without ever calling genex.pages.lock, or you have more unlock calls than lock calls'));
    }
    req.aposPageTreeLockDepth--;
    if (req.aposPageTreeLockDepth) {
      return callback(null);
    }
    return self.genex.locks.unlock('genesys-pages:tree', callback);
  };
  ...
  //
  // This method is called for us by the genesys-docs module on update
  ...
  self.docAfterDenormalizePermissions = function(req, page, options, callback) {
    ...
    var criteria = {
      $and: [
        ...
        self.genex.permissions.criteria(req, 'edit-' + page.type)
      ]
    };
    ...
    return self.genex.docs.db.update(criteria, changeSet, { multi: true }, callback);
  ...
  self.newChild = function(parentPage) {
    ...
    if (!pageType) {
      self.genex.utils.warn('No allowed Page types are specified.');
      return null;
    }
    var page = self.genex.docs.getManager(pageType).newInstance();
    _.extend(page, {
      title: 'New Page',
      slug: self.genex.utils.addSlashIfNeeded(parentPage.slug) + 'new-page',
      type: pageType,
  ...
  self.move = function(req, movedId, targetId, position, options, callback) {
    ...
    function body(callback) {
      ...
      function nudgeNewPeers(callback) {
        ...
        self.genex.docs.db.update(mergeCriteria({ path: self.matchDescendants(parent), level: parent.level + 1, rank: { $gte: rank } }), { $inc: { rank: 1 } }, { multi: true }, function(err, count) {
          return callback(err);
        });
      }
      function moveSelf(callback) {
        ...
        if (parent._id !== oldParent._id) {
          var matchOldParentSlugPrefix = new RegExp('^' + self.genex.utils.regExpQuote(addSlashIfNeeded(oldParent.slug)));
      ...
      function trashDescendants(callback) {
        ...
        return self.genex.docs.db.update(mergeCriteria({ path: matchParentPathPrefix }), action, { multi: true }, callback);
      }
    ...
    return async.series(
      [
        ...
        function trashOrUntrashPages(cb) {
          ...
          return self.genex.docs.db.update({ _id: { $in: ids } }, action, {
            multi: true
          }, (err, res) => {
            cb(err);
          });
  ...
  self.deduplicatePages = function(req, pages, toTrash, callback) {
    return async.eachSeries(pages, (page, cb) => {
      ...
      function deduplicate(cb) {
        if (toTrash) {
          return self.genex.docs.getManager(page.type).deduplicateTrash(req, page, cb);
        } else {
          return self.genex.docs.getManager(page.type).deduplicateRescue(req, page, cb);
        }
      }
      function propagate(cb) {
        ...
        return async.eachSeries(descendants, function(descendant, callback) {
          descendant.path = descendant.path.replace(new RegExp('^' + self.genex.utils.regExpQuote(oldPath)), page.path);
          descendant.slug = descendant.slug.replace(new RegExp('^' + self.genex.utils.regExpQuote(oldSlug)), page.slug);
          return self.genex.docs.db.update({ _id: descendant._id }, {
            $set: {
              path: descendant.path,
              slug: descendant.slug
            }
          }, function(err) {
            if (self.genex.docs.isUniqueError(err)) {
  ...
  self.deleteFromTrash = function(req, _id, callback) {
    ...
    function deleteFromTrash(callback) {
      return self.genex.docs.deleteFromTrash(req,
        {
          $or: [
            {
              path: self.matchDescendants(page)
  ...
  self.update = function(req, page, options, callback) {
    ...
    return async.series({
      ...
      update: function(callback) {
        return self.genex.docs.update(req, page, options, callback);
      }
    }, callback);
  };
  ...
  self.serveNotFound = function(req, callback) {
    ...
    return self.callAllAndEmit('pageNotFound', 'notFound', req, function(err) {
      ...
      req.data.suggestedSearch = self.genex.utils.slugify(req.url, { separator: ' ' });
      req.notFound = true;
      ...
      req.browserCall("genex.emit('notfound', ?)", {
  ...
  self.serveDeliver = function(req, err) {
    ...
    if (err) {
      ...
      req.template = self.genex.templates.renderer('templateError');
      req.statusCode = 500;
      providePage = false;
    }
    ...
    if (args.contextMenu) {
      ...
      args.contextMenu = _.filter(args.contextMenu, function(item) {
        ...
        if (self.genex.permissions.can(req, item.permission)) {
          return true;
        }
      });
    }
  ...
  // This method invokes `pushCreateSingleton` to create the `genesys-pages`
  ...
  self.pageBeforeSend = function(req, callback) {
    ...
    if (typeof _.get(req, 'data.page.level') === 'number') {
      self.genex.templates.addBodyDataAttribute(req,
        {
          'genex-level': req.data.page.level
        }
      );
    }
  ...
  self.ensurePathIndex = function(callback) {
    ...
    return self.genex.docs.db.ensureIndex(params, { unique: true, sparse: true }, callback);
  };
  ...
  self.ensureLevelRankIndex = function(callback) {
    ...
    return self.genex.docs.db.ensureIndex(params, {}, callback);
  };
  ...
  self.updateDescendantsAfterMove = function(req, page, originalPath, originalSlug, options, callback) {
    ...
    var matchParentPathPrefix = new RegExp('^' + self.genex.utils.regExpQuote(originalPath + '/'));
    var matchParentSlugPrefix = new RegExp('^' + self.genex.utils.regExpQuote(originalSlug + '/'));
    var done = false;
    var cursor = self.genex.docs.db.findWithProjection(mergeCriteria({ path: matchParentPathPrefix }), { slug: 1, path: 1, level: 1 });
    return async.whilst(function() { return !done; }, function(callback) {
      return cursor.nextObject(function(err, desc) {
        ...
        return self.genex.docs.retryUntilUnique(req, desc, updateDescendant, callback);

        function updateDescendant(callback) {
          return self.genex.docs.db.update(mergeCriteria({ _id: desc._id }), {
  ...
  self.manageOrphans = function(callback) {
    var managed = self.genex.docs.getManaged();
    var distinct;
    ...
    function findUndefined(callback) {
      return self.genex.docs.db.findOne({ type: { $exists: 0 } }, function(err, _undefinedItem) {
        if (err) {
          return callback(err);
        }
    ...
    function patchUndefined(callback) {
      ...
      return self.genex.docs.db.update({
        type: { $exists: 0 }
      }, {
        $set: {
          type: 'WASUNDEFINED'
    ...
    function find(callback) {
      return self.genex.docs.db.distinct('type', function(err, types) {
        if (err) {
          return callback(err);
        }
        distinct = types;
    ...
    function manage(callback) {
      ...
      self.genex.utils.error('* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *');
      self.genex.utils.error('HEY DEVELOPER: the following types exist in your aposDocs collection,');
      self.genex.utils.error('but are not managed by any module. A generic manager is being stubbed in');
      self.genex.utils.error('for each one to keep your site from crashing, but you should remove these');
      self.genex.utils.error('permanently from the database OR manage them properly by:\n');
      self.genex.utils.error('* Listing them as "types" for "genesys-pages", OR');
      self.genex.utils.error('* Providing an appropriate subclass of genesys-pieces.');
      self.genex.utils.error();
      self.genex.utils.error(orphans.join(', '));
      self.genex.utils.error();
      if (_.contains(orphans, 'WASUNDEFINED')) {
        self.genex.utils.error('Some of your docs had no type at all. The type WASUNDEFINED');
        self.genex.utils.error('has been set for them to allow a generic manager to work.');
      }
      self.genex.utils.error('* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *');
  ...
  self.implementParkOne = function(req, item, callback) {
    ...
    return async.series({
      ...
      findExisting: function(callback) {
        var criteria = {};
        var slugWithOrWithoutSlash = new RegExp('^' + self.genex.utils.regExpQuote(item.slug.replace(/\/$/, '')) + '/?$');
        if (item.parkedId) {
      ...
      updateExisting: function(callback) {
        ...
        return self.genex.docs.db.update({
          ...
          $set: self.genex.utils.clonePermanent(item)
        }, callback);
      },
      ...
      insert: function(callback) {
        ...
        if (!parent) {
          return self.genex.docs.insert(req, item, callback);
        } else {
  ...
  self.genex.tasks.add('genesys-pages', 'unpark',
    'Usage: node app genesys-pages:unpark /page/slug\n\n' +
    'This unparks a page that was formerly locked in a specific\n' +
    'position in the page tree.',
    function(genex, argv, callback) {
  ...
  self.unparkTask = function(callback) {
    if (self.genex.argv._.length !== 2) {
      return callback('Wrong number of arguments');
    }
    var slug = self.genex.argv._[1];
    return self.genex.docs.db.update({
      slug: slug
    }, {
      $unset: {
  ...
  function addSlashIfNeeded(s) {
    return self.genex.utils.addSlashIfNeeded(s);
  }
  ...
  // Invoked by the genesys-versions module.
  //
  ...
  self.matchDescendants = function(page) {
    ...
    return new RegExp('^' + self.genex.utils.regExpQuote(addSlashIfNeeded(page.path)) + '.');
  };
  ...
  self.isAncestorOf = function(possibleAncestorPage, ofPage) {
    ...
    var regex = new RegExp('^' + self.genex.utils.regExpQuote(possibleAncestorPage.path + '/'));
  ...
  // Registers a manager for every page type that doesn't already have one via `genesys-custom-pages`,
  // `genesys-pieces-pages`, etc. Invoked by `modulesReady`
  ...
  // Registers a manager for a specific page type that doesn't already have one via `genesys-custom-pages`,
  // `genesys-pieces-pages`, etc. Invoked by `modulesReady` via `registerGenericPageTypes` and
  // `manageOrphans`

  self.registerGenericPageType = function(type, callback) {
    var manager = self.genex.docs.getManager(type);
    if (manager) {
      return setImmediate(callback);
    }
    var typeName = type + '-auto-pages';
    self.genex.define(typeName, {
      extend: 'genesys-custom-pages',
      name: type
    });
    return self.genex.create(typeName, {
      genex: self.genex
    }, function(err, manager) {
      if (err) {
        return callback(err);
      }
      self.genex.docs.setManager(type, manager);
      return callback(null);
    });
  };
  ...
  // bc wrapper for `genex.templates.append('contextMenu', helper)`.

  self.addAfterContextMenu = function(helper) {
    return self.genex.templates.append('contextMenu', helper);
  };
  ...
  self.addPermissions = function() {
    self.genex.permissions.add({
      value: 'admin-genesys-page',
      label: 'Admin: Pages'
    });
    self.genex.permissions.add({
      value: 'edit-genesys-page',
      label: 'Edit: Pages'
    });
  };
  ...
  self.addToAdminBar = function() {
    self.genex.adminBar.add(self.__meta.name, 'Pages', 'edit-genesys-page');
  };
  ...
  // `genesys-workflow` module to create correct absolute URLs
  // for specific locales.

  self.getBaseUrl = function(req) {
    return self.genex.baseUrl || '';
  };
  ...
  // be passed to `genex.modules['genesys-jobs'].start()` on the rowser side to
  // monitor progress.
  //
  ...
  self.batchSimpleRoute = function(req, name, change) {
    ...
    var data = self.genex.schemas.newInstance(schema);
    return self.genex.schemas.convert(req, schema, 'form', req.body, data, function(err) {
      if (err) {
    ...
    function runJob() {
      return self.genex.modules['genesys-jobs'].run(req, one, {
        labels: {
  ...
  self.allowedSchema = function(req, page, parentPage) {
    var schema = self.genex.docs.getManager(page.type).allowedSchema(req);
    ...
    function getLabel(name) {
      ...
      if (!label) {
        var manager = self.genex.docs.getManager(name);
        if (!manager) {
