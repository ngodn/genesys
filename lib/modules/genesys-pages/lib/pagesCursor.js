...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.addFilter('ancestors', {
      ...
      after: function(results, callback) {
        ...
        return async.eachSeries(results, function(page, callback) {
          ...
          var aCursor = self.genex.pages.find(req);
    ...
    // `children` filter implemented by `genesys-pages-cursor`
    ...
    self.addFilter('children', {
      ...
      after: function(results, callback) {
        ...
        var cCursor = self.genex.pages.find(self.get('req')).areas(false).joins(false).orphan(false);
        ...
        _.each(results, function(page) {
          clauses.push({
            $and: [
              {
                path: new RegExp('^' + self.genex.utils.regExpQuote(self.genex.utils.addSlashIfNeeded(page.path))),
