...
module.exports = function(self, options) {
  ...
  self.renderRoute('post', 'editor', function(req, res, next) {
    var type = self.genex.launder.string(req.body.type);
    var verb = self.genex.launder.string(req.body.verb);
    var id = self.genex.launder.id(req.body.id);
    var schema;
    var parentPage;
    if (verb === 'update') {
      var trash = self.genex.docs.trashInSchema ? null : false;
      return self.find(req, { _id: id }).ancestors(true).permission('publish-doc').trash(trash).toObject(function(err, page) {
        ...
        self.genex.schemas.bless(req, schema);
        return succeed();
      });
    } else if (verb === 'copy') {
    ...
    function succeed() {
      return next(null, {
        ...
        data: {
          schema: schema,
          verb: verb,
          trashInSchema: self.genex.docs.trashInSchema
        }
      });
    }
  });
  ...
  self.apiRoute('post', 'fetch-to-insert', function(req, res, next) {
    // parent id
    var _id = req.body._id;
    var type = self.genex.launder.string(req.body.type);
    return self.find(req, { _id: _id }).permission('edit-genesys-page').toObject(function(err, parentPage) {
      if (err) {
        return next(err);
      }
      if (!parentPage) {
  ...
  self.apiRoute('post', 'insert', function(req, res, next) {
    var parentId = self.genex.launder.id(req.body.currentPageId);
    ...
    return async.series({
      findParent: function(callback) {
        return self.find(req, { _id: parentId }).permission('publish-genesys-page').toObject(function(err, _parentPage) {
          if (err) {
            return callback(err);
          }
          if (!_parentPage) {
      ...
      convert: function(callback) {
        ...
        return self.genex.schemas.convert(req, schema, 'form', page, safePage, callback);
      },
  ...
  self.apiRoute('post', 'fetch-to-update', function(req, res, next) {
    var _id = self.genex.launder.id(req.body._id);
    var type = self.genex.launder.string(req.body.type);
    if (!self.genex.docs.getManager(type)) {
      return next('notfound');
    }
    return self.find(req, { _id: _id }).ancestors(true).permission('edit-genesys-page').trash(self.genex.docs.trashInSchema ? null : false).toObject(function(err, page) {
      if (err) {
        return next('error');
      }
      if (!page) {
  ...
  self.apiRoute('post', 'update', function(req, res, next) {
    var id = self.genex.launder.id(req.body.currentPageId);
    var page = req.body.page || {};
    if (typeof (page) !== 'object') {
      // cheeky
      return next('notfound');
    ...
    return async.series({
      find: function(callback) {
        return self.find(req, { _id: id }).ancestors(true).permission('publish-genesys-page').trash(self.genex.docs.trashInSchema ? null : false).toObject(function(err, _page) {
          if (err) {
            return callback(err);
          }
          if (!_page) {
      ...
      convert: function(callback) {
        ...
        var manager = self.genex.docs.getManager(self.genex.launder.string(page.type || existingPage.type));
        if (!manager) {
          // sneaky
          return callback('notfound');
        }
        ...
        return self.genex.schemas.convert(req, schema, 'form', page, existingPage, callback);
      },
      ...
      findAgain: function(callback) {
        // Fetch the page. Yes, we already have it, but this way all the cursor
        // filters run and we have access to ._url
        return self.find(req, { _id: existingPage._id }).published(null).trash(self.genex.docs.trashInSchema ? null : false).toObject(function(err, _page) {
          if (err) {
            return callback(err);
          }
          if (!_page) {
  ...
  self.apiRoute('post', 'fetch-to-copy', function(req, res, next) {
    var _id = self.genex.launder.id(req.body._id);
    var type = self.genex.launder.string(req.body.type);
    ...
    return self.find(req, { _id: _id }).ancestors(true).permission('edit-genesys-page').toObject(function(err, page) {
      if (err) {
      ...
      if (currentSlugTitle !== self.genex.utils.slugify(page.title)) {
        slugComponents[slugComponents.length - 1] = self.genex.utils.slugify(page.title);
  ...
  self.apiRoute('post', 'copy', function(req, res, next) {
    var currentPageId = self.genex.launder.id(req.body.currentPageId);
    var page = req.body.page || {};
    if ((typeof page) !== 'object') {
      // cheeky
      return next('invalid');
    ...
    return async.series({
      findParentAndSibling: function(callback) {
        return self.find(req, { _id: currentPageId }).permission('publish-genesys-page').ancestors({ permission: false }).toObject(function(err, currentPage) {
          if (err) {
            return callback(err);
          }
          if (!currentPage) {
      ...
      convert: function(callback) {
        var manager = self.genex.docs.getManager(self.genex.launder.string(page.type));
        if (!manager) {
          // sneaky
          return callback('notfound');
        }
  ...
  self.apiRoute('post', 'move', function(req, res, next) {
    return self.move(req, self.genex.launder.id(req.body.movedId), self.genex.launder.id(req.body.targetId), self.genex.launder.string(req.body.position), function(err, changed) {
      if (err) {
        return next(err);
      }
  ...
  self.apiRoute('post', 'move-to-trash', function(req, res, next) {
    return self.moveToTrash(req, self.genex.launder.id(req.body._id), function(err, parentSlug, changed) {
      if (err) {
        return next(err);
      }
  ...
  self.apiRoute('post', 'rescue-from-trash', function(req, res, next) {
    return self.rescueInTree(req, self.genex.launder.id(req.body._id), function(err, parentSlug, changed) {
      if (err) {
        return next(err);
      }
      return next(null, { changed: self.mapMongoIdToJqtreeId(changed), parentSlug: parentSlug });
  ...
  self.apiRoute('post', 'delete-from-trash', function(req, res, next) {
    return self.deleteFromTrash(req, self.genex.launder.id(req.body._id), function(err, parentSlug) {
      if (err) {
        return next(err);
      }
  ...
  self.apiRoute('post', 'get-jqtree', function(req, res, next) {
    return self.find(req, { level: 0 }).children({ depth: 1000, published: null, trash: null, orphan: null, reorganize: true, areas: false, permission: false }).published(null).trash(null).reorganize(true).toObject(function(err, page) {
      ...
      function pageToJqtree(page) {
        var info = {
          ...
          publish: (page.path === '/trash') || self.genex.permissions.can(req, 'publish-genesys-page', page),
          edit: self.genex.permissions.can(req, 'edit-genesys-page', page)
        };
          ...
          _.each(page._children, function(child) {
            if (child.trash && (child.type === 'trash')) {
              ...
              if (self.genex.docs.trashInSchema) {
                forJqtree.label = self.genex.i18n.__('Legacy Trash');
              }
  ...
  self.renderRoute('post', 'chooser-modal', function(req, res, next) {
    return next(null, {
      template: 'chooserModal'
    });
  });

  self.apiRoute('post', 'info', function(req, res, next) {
    return self.find(req, { _id: self.genex.launder.id(req.body._id) }).published(null).areas(false).toObject(function(err, page) {
      if (err) {
        return next(err);
      }
