...
module.exports = function(self, options) {
  ...
  self.apiRoute('post', 'fetch-to-update', function(req, res, next) {
    ...
    return self.find(req, { _id: _id }).ancestors(true).permission('edit-apostrophe-page').trash(self.genex.docs.trashInSchema ? null : false).toObject(function(err, page) {
      if (err) {
        return next('error');
      }
      if (!page) {
  ...
  self.apiRoute('post', 'update', function(req, res, next) {
    ...
    return async.series({
      find: function(callback) {
        return self.find(req, { _id: id }).ancestors(true).permission('publish-apostrophe-page').trash(self.genex.docs.trashInSchema ? null : false).toObject(function(err, _page) {
          if (err) {
            return callback(err);
          }
          if (!_page) {
    ...
    return self.find(req, { level: 0 }).children({ depth: 1000, published: null, trash: null, orphan: null, reorganize: true, areas: false, permission: false }).published(null).trash(null).reorganize(true).toObject(function(err, page) {
      ...
      function pageToJqtree(page) {
        var info = {
          ...
          publish: (page.path === '/trash') || self.genex.permissions.can(req, 'publish-apostrophe-page', page),
          edit: self.genex.permissions.can(req, 'edit-apostrophe-page', page)
        };
