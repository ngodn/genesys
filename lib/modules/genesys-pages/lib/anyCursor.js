...
// subclassing genesys-cursor. This way all pages
    ...
    self.addFilter('pageUrl', {
      ...
      after: function(results) {
        _.each(results, function(result) {
          if (result.slug && self.genex.pages.isPage(result)) {
            var url = self.genex.pages.getBaseUrl(self.get('req'));
            result._url = url + self.genex.prefix + result.slug;
          }
        });
      }
    });
