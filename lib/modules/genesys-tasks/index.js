...
module.exports = {
  ...
  singletonWarningIfNot: 'genesys-tasks',

  afterConstruct: function(self) {
    self.genex.on('runTask', self.run);
  },
  ...
  construct: function(self, options) {
    ...
    //
    // Examples (assume `products` extends `genesys-pieces`):
    //
    // `self.genex.tasks.invoke('genesys-users:add', [ 'admin', 'admin' ]).then(function() { ... })`
    //
    // `self.genex.tasks.invoke('products:generate', { total: 20 }).then(function() { ... })`
    //
    ...
    //
    // During the execution of the task, `self.genex.argv` will have a new,
    // temporary value to accommodate tasks that inspect this property directly
    // rather than examining their `argv` argument. `self.genex.argv` will be
    ...
    self.invoke = function(name, args, options, callback) {
      const aposArgv = self.genex.argv;
      ...
      function body(callback) {
        ...
        self.genex.argv = argv;
        var promise = task.callback(self.genex, argv, after);
    ...
    //
    // Your callback function receives `(genex, argv, callback)`. Your
    ...
    // Call `self.genex.tasks.getReq()` to get a `req` object with
    // unlimited admin permissions. Use `self.genex.tasks.getAnonReq()` to get
    ...
    // want `genex.tasks.invoke` (see above).
    //
    ...
    self.run = function() {
      ...
      var task;
      var cmd = self.genex.argv._[0];
      if (!cmd) {
        throw new Error('genex.tasks.run invoked but there is no command line argument to serve as a task name, should never happen');
      }

      if (cmd === 'help') {

        // list all tasks
        if (self.genex.argv._.length === 1) {
          self.usage();
        }

        // help with specific task
        if (self.genex.argv._.length === 2) {
          task = self.find(self.genex.argv._[1]);
          if (!task) {
      ...
      var promise = task.callback(self.genex, self.genex.argv, afterTask);
      if (promise && promise.then) {
        return promise.then(function() {
    ...
    self.getReq = function(properties) {
      var req = {
        ...
        browserCall: self.genex.app && self.genex.app.request.browserCall,
        getBrowserCalls: self.genex.app && self.genex.app.request.getBrowserCalls,
        query: {},
        url: '/',
        session: {}
      };
      ...
      self.genex.modules['genesys-express'].addAbsoluteUrlsToReq(req);
      return req;
    };
