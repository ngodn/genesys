...
describe('Urls', function() {
  ...
  describe('methods', function() {
    describe('test genex.urls.build', function() {
      it('returns a URL unmodified', function() {
        start = (new Date()).getTime();
        assert(genex.urls.build('/events') === '/events');
      });
      it('returns the URL "#" unmodified', function() {
        try {
          assert(genex.urls.build('#') === '#');
        } catch (e) {
          console.error(e.stack);
        }
      });
      it('adds a single parameter to a queryless URL', function() {
        assert(genex.urls.build('/events', { tag: 'blue' }) === '/events?tag=blue');
      });
      it('appends a parameter to a URL with a query', function() {
        ...
        assert(_.contains(options, genex.urls.build('/events?page=5', { tag: 'blue' })));
      });
      it('replaces parameters in the URL', function() {
        assert(genex.urls.build('/events?tag=blue', { tag: 'red' }) === '/events?tag=red');
      });
      it('removes parameters', function() {
        assert(genex.urls.build('/events?tag=blue', { tag: null }) === '/events');
      });
      it('correctly allows the last data object to win', function() {
        assert(genex.urls.build('/events', { tag: 'red' }, { tag: 'blue' }) === '/events?tag=blue');
      });
      it('places path properties in the path', function() {
        assert(genex.urls.build('/events', [ 'year', 'month' ], { year: '2013', month: '05', tag: 'red' }) === '/events/2013/05?tag=red');
      });
      it('switches to placing path properties in the query if it encounters a non-slugify-compliant property', function() {
        assert(genex.urls.build('/events', [ 'year', 'month' ], { year: '2013!@#@', month: '05', tag: 'red' }) === '/events?year=2013%21%40%23%40&month=05&tag=red');
      });
      it('does the right thing for a case that crashed once', function() {
        assert(genex.urls.build("/events", ["year", "month"], {}, {}) === '/events');
      });
      it('correctly allows the last data object to win for a path property', function() {
        assert(genex.urls.build("/events", ["year", "month"], { year: '2013', month: '01', tag: 'dance' }, { year: 2012, month: '12' }) === '/events/2012/12?tag=dance');
      });
      it('DR use case #1', function() {
        assert(genex.urls.build('/events',
          [ 'year', 'month' ],
          { year: '2013', month: '05', tag: 'dance' },
          { tag: 'tour' }) === '/events/2013/05?tag=tour');
      });
      it('DR use case #2', function() {
        var result = genex.urls.build('/events',
          [ 'year', 'month' ],
          { year: '2013', month: '05', tag: 'dance' },
          { page: '2' });
      ...
      it('IH use case #1: later objects can prevent path properties from being added', function() {
        var result = genex.urls.build('/calendar',
          [ 'year', 'month' ],
      ...
      it('Preserves hashes', function() {
        var result = genex.urls.build('/calendar#skipdown',
          [ 'year', 'month' ],
      ...
      it('Adds to existing query string array when $addToSet is used', function() {
        var result = genex.urls.build('/events?tags[]=purple&tags[]=red', {
          tags: {
            $addToSet: 'blue'
          }
        });
      ...
      it('Adds to existing URI encoded query string array when $addToSet is used', function() {
        var result = genex.urls.build('/events?tags%5B0%5D=purple&tags%5B1%5D=red&tags%5B2%5D=blue', {
          tags: {
            $addToSet: 'green'
          }
        });
      ...
      it('Treats numbers and strings the same when preventing duplicates', function() {
        var result = genex.urls.build('/events?tags[]=4&tags[]=5', {
          tags: {
            $addToSet: 5
          }
        });
      ...
      it('Removes array entirely when $pull removes last item', function() {
        var result = genex.urls.build('/events?tags[]=purple', {
          tags: {
            $pull: 'purple'
          }
        });
